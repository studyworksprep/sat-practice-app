...
test('supplemental import, drafts, duplicates and per-student access',async({page,baseURL})=>{
  ...
  try {
    ...
    await page.getByText('Create a new set',{exact:true}).click();
    ...
    await expect(page.getByText('Set created. You can now choose it as an import destination.')).toBeVisible();
    ...
    await expect(page.getByRole('region',{name:'Question review'})).toContainText('Topic and difficulty metadata are required for the regular bank.');
    ...
    await page.getByRole('checkbox',{name:/Reviewed and ready/}).check();
    await page.getByRole('button',{name:'Import this question',exact:true}).click();
    await expect(page.getByRole('region',{name:'Question review'})).toContainText('Saved as an unpublished draft.');
    ...
    const profile=await admin.from('profiles').select('first_name,last_name').eq('email','student1@test.studyworks').single();
    const studentName=[profile.data!.first_name,profile.data!.last_name].filter(Boolean).join(' ')||'Unnamed account';
    await page.getByRole('searchbox',{name:'Find a user',exact:true}).fill('student1@test.studyworks');
    await page.getByRole('button',{name:'Search users',exact:true}).click();
    await page.getByRole('button',{name:`Grant access to ${studentName}`,exact:true}).click();
    await expect(page.getByRole('list',{name:'People with access'})).toContainText(studentName);
    ...
    if(!await page.getByRole('button',{name:'Compare with question bank',exact:true}).isVisible())await page.locator('summary').filter({hasText:'1. Choose your files'}).click();
    await page.getByRole('button',{name:'Compare with question bank',exact:true}).click();
    ...
    await page.locator('summary').filter({hasText:'Drafts awaiting an answer or final review'}).click();
    ...
    await page.getByRole('button',{name:`Revoke access for ${studentName}`,exact:true}).click();await expect(page.getByRole('list',{name:'People with access'}).getByRole('listitem')).toHaveCount(0);
