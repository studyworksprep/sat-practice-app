...
| 1.5 student invitations | **Done** | 2026-07-16 | Incident response (a shared multi-use tutor code granted a stranger free access; account revoked same day): sponsored student intake moved to **admin-issued, single-use, email-bound invitation codes** (`student_invite_codes`, migration `20260716210000` — **prod apply pending owner**). Invite from /admin/users (email + tutor → code + welcome email); tracker on /admin/users/codes (claimed when/by whom/tutor). The multi-use `teacher_invite_code` is roster-only and **rejected for Studyworks tutors** at signup AND in-app; outside tutors keep it (their students pay). Full matrix verified live in dev; e2e 33/33 |
