...
- **`student_notes`** — student-private rich-text notes (TipTap doc + plain-text projection), optionally linked to a question. Powers the `/notes` hub. A single notes inbox spans both test types; the loader picks the right questions table for the per-note "this is about question X" link.
- **`question_notes`** — tutor-authored annotations on questions, visible to teachers/managers/admins. A tutor reviewing an ACT question can leave a note via the same surface as for SAT.
- **`question_error_notes`** — student "why I got this wrong" reflections that the Review error-log surface aggregates. Separate from `student_notes` and `question_notes`. Same inheritance pattern.
- **`desmos_saved_states`** — saved calculator state per (user, question). ACT math questions accept calculators; same machinery.

(Marked-for-review state already lives in shared, `test_type`-aware columns: `practice_sessions.marked_positions` for free-practice sessions and `practice_test_item_attempts_v2.marked_for_review` for SAT practice tests. ACT practice tests can carry marks on `act_practice_test_attempts` if needed. The legacy `question_status` table — global per-(user, question_v1) flags — is **not** in the shared set: it's keyed to v1 `questions.id`, used only by legacy `/api/*` routes, and retires in Phase 6.)
...
**Audit-greppability.** Every existing call site that reads a shared table (`from('practice_sessions')`, `from('student_notes')`, `from('question_notes')`, `from('question_error_notes')`, `from('desmos_saved_states')`, `from('assignments_v2')`, `from('assignment_students_v2')`) is reviewed once when ACT integration lands and either gains a `.eq('test_type', 'sat')` filter or is explicitly marked cross-test. Joins that enrich with question metadata branch on `test_type` to pick the right questions table. The Phase 2 ACT-integration PR includes this audit list as a checklist — no shared-table reader merges without an explicit test-type story. The audit's scale: roughly **130 readers across ~57 files** as of the initial grep (`question_status` excluded once confirmed legacy-only). The failure mode it guards against is silent: UUIDs in `question_id` columns are globally unique (so the row identifies correctly), but the *join target* differs, so a missed join produces null fields rather than a loud error.
