...
progressive hints, technique tagging — and it does not restate the tutor
...
| 1 — Make checks mean something | **In progress** (2026-08-21) | **1.1–1.3 implemented 2026-08-21** (one PR). 1.1: `check_answers` entries now carry the full attempt history — `{selected, correct, first_try_correct, attempts:[{selected, at}]}` — built by a shared reducer (`applyCheckAttempt` in `lib/lesson/runtime-progress.mjs`) that both the slideshow's local state and the progress actions run; wrong attempts on retry checks and gated Desmos blocks persist with `markComplete:false` so the block stays out of `completed_blocks` and the Continue gate holds across reload (verified end-to-end on studyworks-dev: wrong attempt → reload → selection restored un-revealed, gate locked, attempt in the DB). Legacy two-key entries still read correctly everywhere. 1.2: completion banner now reports first-try accuracy ("You got N of M checks right on the first try"), lists 3+-try steps, and carries a per-check breakdown behind a disclosure — copy is a draft pending 1.7. 1.3: lesson-assignment tutor surfaces — roster page gains per-student progress bar + first-try badge + View report link, a cohort First-try accuracy tile, and a "Lesson checks" struggled-blocks rollup (struggled = 3+ tries or final wrong); the per-student report page renders a per-check table (step, label, first try, tries, final) from `lesson_progress`, with graceful never-started and legacy-entry states. Report logic in `lib/lesson/progress-report.mjs` with unit tests; components in `app/(tutor)/tutor/assignments/[id]/LessonProgressSections.tsx`. Also fixed in passing: a lesson with zero blocks crashed the slideshow (null `.open` read on the calculator override). **1.7 decided 2026-08-22 (owner):** reveal after 2 misses; reveal text = new `solution` field (worked solution, `explanation` fallback); the transfer-check list in `lesson-transfer-checks-proposal-2026-08.md` confirmed as proposed (60 transfer + 31 retrieval designations, all ⚠ judgment calls included). **1.4 + 1.5 implemented 2026-08-22** (one PR). 1.4 runtime: retry checks escalate on the second miss — the worked `solution` (fallback `explanation`) is revealed in an accent-bordered box (newlines render as step breaks), the block completes as struggled (`correct:false` in the attempt entry, so the banner, tutor heat-map, and first-try stats all see it), and Continue unlocks; the miss count restores from the persisted attempt history so a reload can't reset the countdown (verified end-to-end on studyworks-dev). Plumbing: `solution` + `max_attempts_before_reveal` validated in `lesson-validation.mjs` (non-empty string; positive integer; warning when `solution` sits on a one-shot check where it never shows), an editor field in the check form, generator-prompt + authoring-guide §2d/§3b updates; the importer needed nothing (`raw_block` content passes through). Banner copy now reads "Worth another look: Step N" covering both 3+-try and finalized-wrong blocks. 1.5: `scripts/apply-one-shot-checks.mjs` flipped all 91 designated checks to `allow_retry: false` (surgical text edits; parse-level verification that nothing else changed); all 32 specs still validate with zero warnings. Also landed: worked `solution` texts drafted for 72 retry checks across 19 specs (the practice/guided/chain checks plus computation-heavy appliers; agent-drafted, keys re-verified independently, answer-letter cross-checked against `correct_index` at apply time). Tutor-flagged revision 2026-08-22: 15 solutions in the six Desmos-method lessons walked by-hand algebra or verified by substitution where the lesson's point is the calculator workflow — rewritten technique-first in each lesson's own vocabulary (RMSE confirmation only where the lesson teaches RMSE; "read the assigned value" for custom regression; "Desmos displays" for standard regression; intercept labels for x-intercepts; "Desmos returns" for list tools), and the technique-fidelity rule added to the authoring guide §3b and the generator prompt — **1.8 complete 2026-08-22:** the tutor team reviewed all 72 drafted solutions on a generated review page (prompt, choices with the key marked, and hint / two-miss reveal / explanation side by side, LaTeX pre-rendered — rebuildable from the specs) and approved them with no changes. **Re-import complete 2026-08-25:** all 32 edited specs imported into production via Admin → Import (replace) — owner-approved, AI-driven through the admin UI in the owner's authenticated browser session; verified per lesson against the specs by DB query (91 one-shot checks and 72 worked solutions now live; the one block-count difference, Factor Polynomials 45 vs 39, is the two `branching_question` templates expanding to four blocks each at compile time — correct). The production "Bracket the Pivot" in-app draft (updated 08-24, no content ids/flags/solutions) was replaced by the spec per the owner's more-complete call; its distinctive voice-style opener and street-tree opening example are preserved in the session snapshot for Phase 6. Discovered during verification: **four production lessons have no repo spec** — the Phase 0 pair ("Testing Equivalent Expressions with Desmos Sliders", "Find Missing Constants in Equivalent Expressions with Regression") plus two new in-app drafts already using the 1.4 fields ("Circle Toolkit: Measure, Arcs, and Equations", "Initial Modifiers: Match the Noun After the Comma") — reverse-drift to reconcile; also Similar Triangles is now published (Phase 0 carry-forward closed) and Special Right Triangles remains a draft pending a publish decision. **Remaining:** 1.9 pilot (content is now live for it); 1.6 numeric entry parked for Phase 3. **1.6 implemented 2026-09-03:** numeric-entry checks. `check` content takes `input: "numeric"` with `answer` (as a student would type it: a whole number, decimal, or a/b fraction), optional `accept` (equivalent typeable forms) and `tolerance` (≥ 0, for rounded answers) — no `choices`/`correct_index`, and no migration (block_type stays `check`). **Grading is the bank's own SPR grader.** It existed as two identical copies (`session-actions.ts`, `practice-test/grading.js`); now extracted to `lib/practice/spr-grade.mjs` with characterization tests and shared by both callers and the lesson runtime, so a lesson check accepts exactly what a bank question accepts — fractions ≡ decimals, acceptable-answer lists, tolerance — one definition of "right", not three. Runtime: the CheckBlock renders a typed-answer input (mirroring the runner's SPR input: `inputMode=decimal`, the fraction/decimal hint, Enter submits) in place of the choice list; retry, hint, the two-miss solution reveal, escalation, restore-on-reload, and the banner/tutor stats all flow unchanged, plus a graded-result line that shows the keyed answer on a miss. Editor: an Answer-format select (multiple choice / numeric entry) with answer, also-accept, and tolerance fields and inline validation; the preview renders the typed answer. **Found in passing: the validator had no check-shape rules at all** — a one-choice check or an out-of-range `correct_index` reached the runtime silently — so this adds `check_choices_invalid`, `check_correct_index_invalid` (a missing index is a warning: the runtime treats it as 0), and `check_numeric_invalid`; the corpus was already clean. Linter: numeric checks skip the choice rules (the hint-gives-answer rule now covers both surfaces), stats report the numeric count, and a new `lint_spr_candidate` flags one-shot checks whose every choice is a bare number — ratios and percents excluded, since neither is a student-produced-response answer. **Content pass:** the rule flagged 23 checks across 13 lessons; all 23 converted, every math `authentic_item` among them, by a line-level splice (13 files, 50+/161−; each block loses its choices block and gains two lines). The two ratio-answer one-shots (Scale Factor `practice_volume_to_area` / `practice_area_to_volume`) stay MC by design, and the 46 retry checks with numeric choices keep theirs — there the distractors and the hint are the teaching. Corpus after: 611 MC + 23 numeric, 63 lint warnings (unchanged), keyed-longest 20.1% over the 611. Docs: guide §2d (answer format is part of SAT authenticity), §3b (the shape and when to use it), §6; generator prompt; skill step 5; the authenticity sheets' "MC-only until 1.6" lifted; punch-list rules + note. Verified: 453 unit tests (grader characterization, numeric-check helpers, validator, linter), typecheck, lint, hygiene; the typed-answer surface exercised in the browser against the real slideshow on a no-auth harness page (retry → hint → second miss → solution reveal with the keyed answer; a one-shot fraction accepted in its reduced form; Enter submits), harness deleted before commit. **Phase 1 AI steps are now all complete;** 1.9 (the pilot) remains the open tutor task. **Found 2026-09-03 (owner report):** a finished lesson never reached its *assignment* — `assignment_students_v2.completed_at` was stamped only by the practice-session paths, so the assignments list, the student's assignment pages, the tutor dashboard feed, and team effectiveness all showed lesson assignments as never completed even though `lesson_progress` and the 1.3 roster/report had it (production: 4 finished, 0 marked). Fixed: Complete Lesson stamps every open assignment of that lesson (`lib/lesson/assignment-completion.ts`), assigning a lesson a student already finished stamps it on arrival with the lesson's own completion time, and a completed single-student lesson tile opens the per-student lesson report. The 4 existing rows were backfilled on owner approval. |
| 2 — Item-quality pass | **In progress** (2026-08-23) | **2.1 + 2.2 implemented 2026-08-23** (one PR). 2.1: linter logic in `lib/lesson/lesson-lint.mjs` (10 rules per the plan, each unit-tested on review-style defects and a clean control), per-spec CLI at `.agents/skills/create-sat-lesson/scripts/lint-lesson-spec.mjs`, and `scripts/check-lesson-specs.mjs` wired into CI — validator errors now fail the build (new gate; specs were previously unchecked in CI) while lint findings print as a report and never fail. Tuned against the corpus to remove three false-positive classes: LaTeX command names are preserved in normalization (`\tan` vs `\cos` are different answers), choice-text equality is case-sensitive (`x=6` vs `X=6` is the Desmos variable-naming distinction), and equations carry no bare numeric value (`y=3` ≠ `a=3`). 2.2: punch list generated at `lesson-lint-punch-list-2026-08.md` — **265 findings across 32 specs / 540 checks**; corpus keyed-longest rate **35.4%** (exit target < 30%), worst lesson Inference at 91%; by rule: keyed_longest 107, extreme_imbalance 58, check_run 20, key_term_echo 20, text_run 18, retrieval_nonsense_distractor 15, missing_figure 11, hint_gives_answer 9, equivalent_choices 5 (all five verified real — two are distractors numerically equal to the keyed answer, in the Probability transfer checks), meta_prompt 2. **2.6 seeded 2026-08-25:** `trap-catalog.md` (Living, indexed) drafts the trap catalog from the corpus itself — 209 candidate traps across all 32 lessons, extracted from distractor explanations, hints, worked solutions, and the Good Cop / Bad Cop wrong-choice debriefs, each anchored to the check block ids where a distractor encodes it; every entry is status `candidate` and every name a proposal until the tutor session confirms/renames (the 6.5 names rule applies). **2.7 derived 2026-08-25:** `authenticity-sheets.md` (Living, indexed) replaces the from-memory authoring the plan assumed with empirical derivation — the owner observed the question bank IS authentic College Board Educator Question Bank content, so every sheet is computed from `questions_v2` (3,430 live items), with **PT9–11 as the canonical anchor for currency and operational weighting** (owner decision; 433 distinct recent-test questions). Per skill: bank-vs-recent weighting with drift flags, stem-length percentiles, choice-shape stats, empirically mined stem templates (all seven R&W skills run on near-invariant closing questions — use verbatim in 2.4), difficulty profiles with exemplar `display_code`s per band (item text not reproduced — College Board's). Headline findings: the two Surveys-adjacent PSDA skills appear **zero times on PT9–11**; SPR is 24% of bank math (supports keeping 1.6); hard math items run 30–60% longer than easy ones. Queries preserved in `scripts/authenticity-sheets-queries.sql` for regeneration when the bank updates. **Tutor verification pass remaining:** format drift since PT11, divergence-vs-noise calls, and drill curation. **2.5 complete + Probability correctness fixes 2026-08-26** (one PR): all 32 final-retrieval checks (the 31 designated retrieval blocks plus Good Cop's `final_wrong_choice_check`) rebuilt — every distractor is now the lesson's own process with steps swapped, omitted, or a rule inverted, at comparable length in the lesson's vocabulary; keys and `correct_index` untouched (raw-text splice with parse-level verification that only the choices arrays changed). Also fixed the three Probability from Tables `equivalent_choices` defects: block 31 60/100→39/100 (grand-total error, no longer ≡ key 39/65), block 32 14/35→14/100 and 26/65→26/100 (single-cell-favorable errors, no longer ≡ key 40/100) — both were live one-shot correctness bugs — and block 22 24/40→6/40 (joint-vs-conditional error, B no longer ≡ C). Result: `retrieval_nonsense_distractor` 15→0, `equivalent_choices` 5→2 (the Scale Factor pair remains for 2.3/2.6), corpus keyed-longest 35.4%→31.1%, punch list 265→209 open (56 rows flipped to fixed); all 32 specs validate zero-warning and the CI gate passes. Production still carries the old items until the next owner-approved re-import. **Reverse-drift reconciliation complete 2026-08-26** (same PR): the four spec-less production lessons now have repo specs — `testing-equivalent-expressions-with-desmos-sliders.json` (22 blocks, published), `find-missing-constants-in-equivalent-expressions-with-regression.json` (33, published), `circle-toolkit-measure-arcs-and-equations.json` (43, draft), `initial-modifiers-match-the-noun-after-the-comma.json` (30, draft) — closing the Phase 0/1 carry-forward. Content was pulled from production via md5-verified base64 block dumps and each generated spec compiles to a block sequence that deep-equals the production dump exactly (text blocks use the `text` sugar; everything else is `raw_block` pass-through; `calculator` lifted to the spec level per corpus style; each spec carries an `authoring_notes` provenance line). All four validate zero-warning; the corpus gate now covers 36 specs / 600 checks. Lint on the newcomers: the two in-app drafts are clean; the two older published lessons add 21 open punch-list rows (Sliders is 57% keyed-longest — a 2.3 rewrite candidate) and have no transfer/retrieval designations yet — extending the 1.5/1.7 designation pass to them is a new open item for the tutors. **2.6 + 2.7 closed 2026-08-26 by owner blanket confirmation** — the owner confirmed both review artifacts as drafted rather than running the per-row tutor session / sheet-by-sheet pass the plan sketched: all 209 trap rows (names, scopes, the 10 family names) are status `confirmed`, check-artifact rows stay as documented-never-drafted, and the authenticity sheets stand as computed (no drift flagged since PT11; divergences stand as reported). Both remain Living — rename/delete on real student observation. The two 2.7 open questions were decided: **TS&P is hosted by `reading-comprehension-process-and-pre-answer`** (verbatim function stem; dedicated lesson stays a Phase 4 candidate), and **2.4 math items are MC-only until 1.6** (SPR variants added later, SPR-heavy skills first). Still uncatalogued/undesignated: the four reverse-drift lessons. **2.3 tranche 1 complete 2026-08-26** (same PR as the 2.6/2.7 closure): all `hint_gives_answer`, `meta_prompt`, and `equivalent_choices` rows resolved — 12 fixes (hints rewritten to point at the deciding step without stating the answer; the Probability model prompt reframed off "Why is X correct"; the two Scale Factor duplicate distractors re-encoded as confirmed traps: `3/2` change-over-original and `4:25` inverted area ratio) plus 3 reasoned deferrals (Good Cop's wrong-choice debrief is the lesson's taught technique; Sliders' "p=2" is the stated test value, not the answer; Inference's retrieval echo is the keyed process itself). Also the full **Inference lesson rebuild** — the corpus's worst at 91% keyed-longest — distractors re-typed across the five confirmed traps at key-comparable length with at most one extreme-quantifier choice per item (killing both the length tell and the hedged-key tell): 18 lint warnings → 1 deferral, keyed-longest 45%. **Corpus keyed-longest is now 29.5% — under the <30% exit target**; punch list stands at 198 open / 85 fixed / 3 deferred. **2.3 tranche 2 complete 2026-08-26** (one PR): full distractor rebuilds of the six lessons above the 50% bar — CLEAR the Claim 86%→36% (15 warnings→3 structural; `hard_bridge_check` prompt made self-contained, clearing its `missing_figure`), Good Cop 74%→11% (18→2; load-bearing strong-word traps kept — they anchor the wrong-choice debrief checks — while incidental extreme words were reworded; debrief-check distractors became plausible mis-diagnoses), Process and Pre-Answer RC 81%→13% (zero warnings), Rhetorical Synthesis 78%→33% (zero; the exploration and transfer text blocks' candidate lists updated in lockstep with their checks), Surveys 63%→13%, Sliders 57%→14% (its `retrieval_nonsense_distractor` cleared). Method as tranche 1: keys and `correct_index` untouched, distractors re-typed across the lessons' confirmed traps at key-comparable length, ≤1 listed-extreme-word distractor per item, explanations verified compatible (one reworded). **Both lint exit criteria are now met: corpus keyed-longest 22.0% (<30%) and highest lesson 47% (<50%).** Punch list: 144 open / 139 fixed / 3 deferred — the remainder is the scattered long tail plus structural classes. **2.8 preview sign-off 2026-08-26:** the owner previewed the rebuilt content as a student and approved proceeding with the remaining changes. **2.4 complete 2026-08-26** (one PR): every lesson now has ≥1 authentic-format item — 34 items authored and inserted as one-shot checks (block id `authentic_item`, placed before each lesson's final retrieval check), while CLEAR the Claim and Desmos Sliders already carry real bank items via `question_link`. Each item uses its skill's verbatim near-invariant stem, sits in the sheet's p25–p75 length band, matches the skill's choice shape (1-word WIC, punctuation chunks for Boundaries, ~20-word Rhetorical Synthesis sentences, mostly-numeric math), and draws its distractors from the lesson's confirmed test-scope traps; explanations model each lesson's taught workflow. Per the owner decisions: TS&P is hosted in `reading-comprehension-process-and-pre-answer` (the structure stem, which needs no underlining) and all math items are MC with the calculator open (Desmos is always available on the real section). Self-review caught and fixed one drafting bug: a Factor-out-GCF distractor that expanded to an expression equivalent to the original. Corpus after insertion: 634 checks, keyed-longest 21.3%, highest lesson 44% — the authentic items themselves are lint-clean; `check_run` grew 21→35 because items sit adjacent to retrieval checks (structural class, Phase 3 pacing). **Long-tail sweep complete + punch list CLOSED 2026-08-26** (same PR as 2.4): 76 more blocks across 24 lessons fixed — distractors lengthened to key-comparable length, extreme-word clusters reduced to ≤1 per item, echoed lesson terms placed into a distractor — taking `keyed_longest` 52→**0** and leaving `extreme_imbalance` / `key_term_echo` only as reasoned deferrals (the Special Systems and subject–verb rows whose choices are the lessons' own taxonomy labels). Structural classes (`check_run`, `text_run`, `missing_figure`) are class-deferred to Phase 3 where pacing rework and 3.1 are scheduled. Punch list final state: **229 fixed / 57 deferred / 0 open**. **Phase 2 exit criteria are all met:** every row fixed or deferred with a reason; corpus keyed-longest 19.4% (<30%); highest lesson 44% (<50%); every lesson has ≥1 authentic-format item; linter in CI since 2.1. **Production re-import complete 2026-08-27, closing the phase:** all 36 specs imported via Admin → Import (replace) in the owner's authenticated browser session (owner-approved, AI-driven) and verified byte-exact 36/36 by md5 of a canonical serialization of each lesson's production blocks against the locally compiled specs. The rebuilt corpus — one-shot checks, worked solutions, rebuilt distractors, authentic items — is what students now see. |
| 3 — Use the medium | **In progress** (2026-08-27) | **3.1 implemented 2026-08-27** (full pinned-figure support; the interim img-repetition pass was skipped as superseded). Any block's content may carry `figure` {src, alt, caption}: validated (`lib/lesson/figure.mjs` + `figure_invalid` in the validator), lifted from spec-block level by the importer like `calculator`, editable in the block editor (FigureEditor panel), and rendered by the slideshow as a persistent side-pane card — stacked above the calculator when both are visible (stickiness dropped in that case so they can't scroll over each other), swapped to an inline card above the block at ≤800px. The lint `missing_figure` rule accepts a `figure` as satisfying a reference; authoring guide gains §3f. Content pass: 9 figure attachments across Similar Triangles (5: steps 3/4/10/12/13, the angle-correspondence diagram), Scale Factor (area check + volume check, with answer-revealing alt text neutralized — the original alts stated the ×9/×8 conclusions the checks ask for), Standard Deviation (spread check), and GCF (group check). `missing_figure` 10→2; the two remaining are reclassified punch-list deferrals (Trig 21's "the picture" is generic sketch advice; Special Systems 14's is the student's own Desmos graph). Verified on studyworks-dev: spec imported through Admin → Import, side-pane figure + caption render on check and text slides, mobile inline swap works, all 378 unit tests + typecheck pass. Generator-prompt support deliberately skipped: the gen path already uses `figure` as its declarative-geometry *kind* — wiring the pinned key into generated specs needs a naming decision first. **3.3 implemented 2026-08-27:** duration estimated from the block mix at read time (`lib/lesson/duration.mjs`; text 0.6 / check 1 / Desmos 2 / question_link 2 / video 1.5 / lesson_complete 0.3 min, display rounded to the nearest 5 with a floor of 5) — shown right-aligned on the Learn catalog card and in the lesson-viewer header meta row ("~25 min"; the "N of M in unit" position waits on Phase 5 ordering). The catalog query fetches block types chunked 20 lessons per request to stay under PostgREST's 1000-row cap. Verified on studyworks-dev as the seeded student: card and header both render "~5 min" for the 5-block fixture lesson. **3.5 done 2026-08-27:** the `video` block's student-runtime embed is confirmed in code (YouTube via youtube-nocookie, Vimeo via player.vimeo.com; non-embeddable URLs fall back to a link) — with one real gap found and fixed: an unlisted Vimeo link's privacy hash (`vimeo.com/id/hash`) was being dropped, which the player rejects; the runtime and the editor preview now pass it as `?h=`. The 60–90 s intro-video-as-block-1 convention (face + whiteboard: the move, the payoff, one caution) is in the authoring guide's video section and the generator prompt's `video_placeholder` line. Recording (3.6) is the tutor workstream. **3.2 implemented 2026-08-27:** Desmos regression/table validation, designed against a live calculator rather than from the docs — the decisive finding is that `getState()` exposes `regressionParameters` on the regression row itself, so validation stays **synchronous** (no async `HelperExpression` plumbing; the sync read returns nothing while the pane is hidden, because Desmos pauses its compute loop). New in `desmos-interactive.mjs`: `state_rules.require_table` (`{columns, rows, tolerance}`; rows match order-insensitively and ignore Desmos's trailing blank row), `state_rules.require_regression` (a row using `~`), and `validation.expected_parameters` (`[{name, value, tolerance}]`, default tolerance 0.01) — with five new targeted-hint triggers (`missing_table`, `table_rows_mismatch`, `missing_regression`, `missing_parameter`, `parameter_mismatch`) ordered structurally, so the hint names the step that is actually wrong. The runtime now passes tables and fitted constants through `extractStudentExpressions` (tables carry no latex, so they were previously dropped) and shows the per-parameter report under debug. 15 unit tests on fixtures captured from the live calculator. Gates authored in **12 blocks across 4 lessons**: Custom Regression 5 (steps 4/15/23/27/33) and Standard Regression 5 (steps 2/16/22/30/32) — each converting the instruction text block into the activity that verifies it, so wording and block counts are unchanged — plus `expected_parameters` added to the two lessons that already gated (Solving Equations 11, Functions 31). Every expected value was confirmed against a live fit (a=3; b=5; c=8; r=0.1; u=4; q=7; c=3). **Three authoring rules the implementation forced, now in guide §3g and the generator prompt:** only name a constant Desmos itself solves for (a definition row like `a=10-b` means Desmos reports `b` only); never gate on a value the fit does not pin down (Solving Equations 33 is deliberately value-free — `u+v=13` with `u²+v²=85` has two valid roots); and gate button-driven Standard Regression on table+regression only, since the Regression button's model form and constant names are Desmos's (it does not respond to synthetic clicks, so its naming could not be verified). Every gate carries `reveal_solution_after_attempts: 3` + a workflow-vocabulary `solution_html`, so `require_success` can never hard-stick a learner. Verified end-to-end on studyworks-dev: empty submit → `missing_table` hint with Continue locked; correct table+regression → Desmos fits a=3, success, Continue unlocks; wrong model (fitting a=2.75) → `parameter_mismatch` hint. Also fixed in passing: the converted blocks told learners to "reset the scratch calculator" that a per-activity calculator no longer has, and their lifted `<h2>` printed the title twice. **3.4 complete 2026-08-27:** interface figures authored as SVG in the established house style rather than captured as literal screenshots — the repo's three existing Desmos figures are all authored SVG (crisp at any size, no Desmos-version drift, and no question about redistributing a third-party UI), so the plan's "browser automation" note was followed in spirit: every figure was drawn from a live readout observed in the embedded pane. **A real content question was settled in the process:** the readout differs by regression type — a *table* regression reports `R²` and `r`, while a *table-free* one (`5q+7~42`) reports `RMSE` — verified on both exact and imperfect fits. The two lessons that teach RMSE (Percentages, Solving Equations with Regression) use exactly the table-free form, so the corpus was already correct; the two readouts are now separate figures so neither lesson shows the wrong one. New: `desmos-regression-readout-table.svg`, `desmos-regression-readout-rmse.svg`, `desmos-percent-of-control.svg` (the `%`→`of` auto-insert, with the multiplication dot drawn as a circle so no font substitution can drop it; arrowheads pinned to `userSpaceOnUse` rather than inheriting the existing figures' oversized stroke-scaled heads). Placed on 15 blocks via 3.1's pinned `figure`: the table readout on all five Custom Regression gates, the RMSE readout on the two RMSE-teaching blocks plus Percentages' `rmse_meaning`, the percent control on `percent_of_input`, the **existing** Regression-button figure moved forward to the five Standard Regression gates (it previously appeared only on a teaching slide *after* the opening exploration told learners to hunt for it), and the **existing** visibility-toggle figure onto Special Systems' `infinite_solution_exploration`, which told learners to click a colored circle it never showed. Verified on studyworks-dev: the readout renders in the side pane stacked above the required-activity calculator, so what to look for sits beside where to look. **3.6 AI half delivered 2026-08-27:** `lesson-intro-video-briefs-2026-08.md` (Living, indexed) — one brief per lesson for all **36**, each naming the move in the lesson's own words, the test-day payoff, and exactly one caution drawn from that lesson's confirmed traps, plus the recording convention (60–90 s, block 1, face + whiteboard, three beats, no agenda) and the unlisted-link mechanics from 3.5. Queued in the plan's priority order with a Recorded checkbox per lesson: Tier 1 is the fourteen lessons the review rated 4, Tier 2 the three section foundations everything else leans on, Tier 3 the remaining nineteen. Recording is the tutor workstream and stays open. **3.9 implemented 2026-09-01 — the step that unblocks the phase.** Recording 60–90 s of face-and-whiteboard for 36 lessons is days of tutor time, and it was the only thing standing between Phase 3 and its exit. It turned out to gate nothing else: no step in Phases 4, 5, or 6 depends on 3.6, and 6.6 already carried it as an ongoing track — so the "≥ 10 intro videos live" exit criterion moved to Phase 6 and recording became a standing track. What ships instead is the *content* of the intro, in text: a `lesson-intro` card carrying the same three beats in the same order (the move, the test-day payoff, exactly one caution), authored from the 36 briefs 3.6 had already written. **All 36 lessons carry one.** The card is not a placeholder — when a recording lands, the `video` block goes in above it and the card stays as the read-it version for learners who don't watch, which is also why it is worth the corpus-wide pass now. Two implementation decisions worth keeping: (1) the card merges into the *existing* orienting block rather than becoming a new block 1, so no block index shifts — the punch list, trap catalog, briefs, and this ledger all reference blocks by 1-indexed position, and `lint_text_run` (21 warnings) would have moved too; every one of the 36 specs already opened with a `text` block, so this cost nothing pedagogically. (2) Placement is not uniform: 30 lessons take the card after block 1's `<h2>`; **Custom Regression** and **Good Cop / Bad Cop** take it after their prerequisite Callout so the gate stays first; and the four cold-open lessons (**Find Missing Constants in Equivalent Expressions**, **Percentages**, **Solving Equations with Regression**, **Initial Modifiers**) take it on block 3, because their block 1 is a discovery activity whose answer *is* the move — naming it above the hook would spoil the exploration. Wording was angled away from each opener's own `<h2>` and goal Callout so the card reads as the tutor's framing rather than a restatement; `scale-factor`, `special-right-triangles`, `testing-equivalent-expressions`, `functions`, and `command-of-evidence` needed rewrites where a beat had landed on their heading verbatim. The linter caught two cards pointing at a picture their slide doesn't carry (Similar Triangles' "the figure says not drawn to scale", Standard Deviation's "straight from the picture") — both reworded, and the corpus is back to its 63-warning baseline with all 36 specs validating. Edits were spliced into the raw JSON rather than reserialized: 21 of the 36 specs carry hand-formatted one-liners that `JSON.stringify` would reflow, and the diff is exactly one line per file. New `.lesson-prose .lesson-intro` styling in `app/globals.css` (quieter than the accent blockquote, which §3a reserves for a single key idea); convention documented in authoring guide §3a with the placement exceptions and the "don't restate the `<h2>`" rule. **Remaining for Phase 3:** the 3.7 tutor pass on the new Desmos gates against the real Bluebook calculator, 3.8 (the Bluebook eliminator screenshot for Words in Context block 25, which needs the Bluebook app), and the owner-approved production re-import of the Phase 3 spec changes. 3.6 recording continues as a standing track and no longer appears here as a phase blocker. |
| 4 — Rebuild and re-scope the laggards | Not started | |
| 5 — Wire lessons into the platform | **In progress** (2026-09-01) | **5.2, 5.3, 5.5 implemented 2026-09-01** — the lane that needed no tutor input. **5.3 (delayed retrieval):** completing a lesson enqueues it in `review_queue` due in ~2 days. The plan's "simpler" option — reuse the existing `skill` item type — turned out to be unsound: `syncDecayedSkillReviews` reconciles skill rows against `get_student_coverage` and *deletes* every one whose unit is no longer `decayed`, so a lesson-originated skill row would be swept before it ever came due (a just-taught skill is the least likely to read as decayed). Hence a distinct `item_type = 'lesson_check'`, invisible to that reconciliation — **which needs a migration the plan did not anticipate** ("Only 5.1 in this plan needs one"), since `item_type` carries a CHECK constraint. No type regeneration: `item_type` is `text`, so the constraint never reaches the generated types. `item_ref` is a lesson uuid; a due item resolves to a micro-drill over the lesson's `skill_code` via `lesson_topics`. Intake is upsert-ignore-duplicates, so re-completing neither stacks rows nor pushes the due date out; the enqueue runs in `after()` and swallows errors, matching the rest of the queue intake. **5.2 (practice at the end):** a "Practice this now" CTA on the completion banner. It could not reuse the Review page's `createSkillDrill` — `buildWeakQueue` returns `[]` with no attempts and otherwise keeps only questions already *missed*, i.e. empty for a just-taught topic, so the student would have been told "No questions to review yet" exactly when they most want practice. New `lib/lesson/practice-drill.ts` selects the lesson's pattern/skill ordered least-recently-attempted first (never-attempted first), sharing one ranking helper with 5.3's micro-drills. Sessions are `mode='practice'`, not `'review'` — a review session bounces to `/review` on completion and would strand a student who came from a lesson. **5.5 (efficacy view):** an "In-lesson" column on `/admin/lessons` beside `feature_efficacy` (that measures practice accuracy pre/post the lesson; this measures the lesson's own checks, which is what separates a hard item from a broken one). Two calls forced by real data rather than the spec: Desmos entries are excluded (a gate's attempt count measures the gate, not the item), and the rate counts only entries written since Phase 1.1 — older ones have no `first_try_correct` and `checkFirstTryCorrect` falls back to final correctness, so **9 of the 13 production lessons with answers have zero tracked entries and read a confident 100%**, which the bands would have flagged as "too easy". They now read "N legacy", and a thin-data guard (n < 10) mutes the bands (without it Solve Equations by Graphing flags "too easy" on n=3). **Production readiness, measured not assumed:** 30 of 36 lessons resolve to real questions, every one yielding a full 8-question drill (smallest pool 24); the 6 that don't have no `skill_code` at all and are all cross-skill strategy lessons (Advanced Factoring, Find the Equation with My Numbers, Good Cop / Bad Cop, Process and Pre-Answer, Solving Equations with Regression, My Numbers) — tagging them is a tutor task, and until then their CTA is correctly hidden and their retrieval item waits rather than being dropped. **2026-09-23:** the question-pattern layer was retired in favor of techniques (`foundations-and-question-patterns.md` §8); the drill now narrows to the lesson's techniques (tagged or default-applicable questions first) and fills from the skill, so it never runs short. **Open:** the `lesson_check` migration is applied to studyworks-dev and verified, **not to production — awaiting owner approval**. 5.1 still waits on the 5.6 prerequisite graph; 5.4 waits on 5.8. |
| 6 — Voice and personality | **In progress** (2026-09-03) | **6.1 done 2026-09-03:** the house voice is a subsection of authoring guide §2c — six rules from the review's §5 (payoff in one line; name the move and reuse it; let the SAT be a character; break the cadence; end hard; use the material's own drama) with the review's two real rewrites as the register examples, plus a §3b rule that `lesson_complete` is a two-line sign-off, never a "You can…" recap. **6.3 done 2026-09-03:** the generator prompt gains a House voice section, its sequence step 10 now ends hard (retrieval in the lesson's voice → the SAT-format item last → sign-off), its tone line carries the point of view, and the `create-sat-lesson` skill's authoring step says the same; the linter adds `lint_retired_tail` ("Next, you will…" previews) and `lint_retired_closer` (the stock "Without looking back, which…" stem) and reports each lesson's same-word explanation-opener rate as a stat. Measured, not assumed: the rules found **28** stock closers, **12** tails, and a **93.9%** same-word opener rate (34 of 36 lessons ≥ 90%). **6.4 done 2026-09-03:** the completion banner rewards the right thing — a "Clean run" or the longest first-try streak (≥ 3), and the measurement items (one-shot checks that aren't the retrieval closer, i.e. the transfer and SAT-format items whose distractors carry the catalog's traps) as "you didn't take the bait on N of M test-style items"; never more than two lines, nothing when nothing was earned. Pure, tested (`completionHighlights`, `firstTryStreak`, `measurement` in `summarizeCheckAttempts`). **6.2 proposed 2026-09-03 — awaiting tutor approval:** `lesson-voice-rewrites-2026-09.md` (Living, indexed) plus a review page, generated from one source so they cannot drift and with every "Was" read from the live spec. Per lesson: a proposed handle (5 already named, 6 in the title, 25 new — e.g. Zero, Graph, Click; Total First; Rank the Sides; Tilde Swap; Same Slope; Fill the Constant / Table, Then Button), a rewritten opener body below the intro card for **21** lessons (the 19 that still carried an agenda sentence — "By the end of this lesson…" — plus Custom Regression's "Here, you will build on…" and the review's own List Tools rewrite), a two-line sign-off replacing the "You can…" recap for **35** of 36 closers (Sliders already signs off in voice), and a fresh retrieval stem for all **28** stock ones. Four blanket decisions for the team: end on the SAT-format item rather than the retrieval recital (a reorder in 32 lessons); drop the three "in one glance" summary blocks; delete the 12 tails; and treat the 587 same-word explanation openers as a separate pass (6.2b) after names are settled, so the rewrites can use the names. **Nothing is applied to a spec until approval;** the 40 new lint rows close with the application. **6.2 applied 2026-09-03** — the owner approved all as proposed, which also settles **6.5**: the 36 handles are decided. `scripts/voice-rewrites/apply.mjs` (dry-run first; every result re-parsed and compared to the intended object before writing; idempotent — a second run is a no-op): 21 openers, 35 closers, 28 retrieval stems, 12 tails deleted, 3 summary blocks removed, and the end-hard reorder in **33** lessons — the 32 proposed plus CLEAR, which the proposal mis-described as already ending on a bank question (it ended on its retrieval check) and now follows the approved principle; Circle Toolkit's and Initial Modifiers' non-stock retrieval checks were treated as the recital they are. 36 files, 666+/684−. Corpus back to 63 warnings with `retired_closer` and `retired_tail` at 0; all specs validate; the unit suite is unchanged, and because no block id changed, existing progress rows, the one-shot designations, and the 5.3/6.4 id conventions all still resolve. **Phase 6 exit status:** voice guide merged ✓; openers/closers rewritten and approved ✓; move names in use in hints and explanations ✗ — that is **6.2b** (the 587 same-word explanation openers), now unblocked because the names exist. Production lessons pick all of this up at the next re-import. **Open:** 6.2b, 6.6 videos (standing), 6.7 texture. |
...
- **2.6 [Tutors, M]** Approve or adjust the AI-drafted distractors — this
  is the judgment step. Keep a shared **trap catalog** per skill / technique
  ("what students actually do wrong here"), which the AI drafts from
  and which also feeds the Techniques catalog
  (`foundations-and-question-patterns.md` §8).
...
- **3.5 [AI, S]** Video convention. Confirm the `video` block embeds
  ...
  path).
- **3.9 [AI, M]** Intro cards. Give every lesson the intro video's three
  beats in text — the move, the test-day payoff, one caution — as a
  `lesson-intro` card on the block that orients the lesson, authored from
  the 3.6 briefs. This is what lets the recording queue stop gating the
  phase: the beats ship now, and the card stays in place under the video
  when a recording lands. Convention in the authoring guide §3a.
...
- **3.6 [Tutors, L, ongoing]** Record 60–90 second intro videos per
  ...
  **This step does not gate Phase 3.** Recording runs as a standing
  track, tracked at 6.6; nothing downstream depends on it, and 3.9's
  cards carry the same three beats in the meantime.
...
the workflow; every catalog card shows a time estimate; every lesson
opens with its intro card; screenshots in place. The "≥ 10 intro videos
live" criterion moved to the Phase 6 exit on 2026-09-01 — the recordings
are a standing track and no longer gate this phase.
...
`feature_efficacy`, and technique tagging.
...
- **5.2 [AI, M]** Practice at the end. On `lesson_complete`, a "Practice
  this now" CTA that launches a drill of the lesson's skill narrowed to
  its techniques (the drill draw in `foundations-and-question-patterns.md`
  §8), filling from the skill when the technique runs short.
...
- **5.3 [AI, M]** Delayed retrieval. On completion, enqueue in
  ...
  skill micro-drill) due in ~2 days; consumed by the existing
...
- **5.7 [Tutors, ongoing]** Question → technique tagging
  (`foundations-and-question-patterns.md` §8). This is what makes
...
**Exit:** prerequisites visible in the catalog; every technique lesson ends
...
and explanations; ≥ 10 intro videos live (3.6, moved here from the
Phase 3 exit on 2026-09-01).
...
| 4 | AI | Phase 3.1–3.5 figures, Desmos gating, time, screenshots, video convention; 3.9 intro cards | Phase 1; 3.9 needs the 3.6 briefs |
| 4 (parallel, non-blocking) | Tutors | 3.6 videos — standing track, gates nothing | AI briefs |
...
| ongoing | AI | Numeric-entry check (1.6) — **done 2026-09-03** | landed after Phase 5.2/5.3/5.5 |
