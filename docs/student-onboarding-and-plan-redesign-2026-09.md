...
| **2c · Unit syllabi** — **schema + generator 2026-09-22** | `curriculum_unit_steps` (lessons in teaching order, each followed by its practice drill, mixed sets; shared lessons taught once; Mark done stamps the lesson complete) walked by the generator behind flag `unit_syllabus`; default two-step syllabi backfilled so output is unchanged until authored. Design and status in `foundations-and-question-patterns.md` §7. **Authoring landed 2026-09-22** (§7.4): an in-app Curriculum area for a non-technical admin — unit list with statuses, card-based unit editor with a searchable lesson picker and a student-sees preview, the plans-use-syllabi switch, tutor "Unit syllabus" add. Migration applied to production 2026-09-22 (flag off). Technique-targeted drills and lesson reassignment follow. | 12 unit tests on the walk (`lib/plan/unit-syllabus.test.mjs`); full suite 574 green; dev walk with an authored H.A. syllabus. Production migration pending. |
| **2d · Techniques layer** — **steps A–D** (A + B 2026-09-23: schema + rename, per-unit tagging screen; C + D 2026-09-24: lesson technique pickers + technique-first practice steps; section foundation syllabi) | Question patterns retired in favor of techniques (`foundations-and-question-patterns.md` §8): `techniques`, `technique_skills` (default applicability by skill), `question_techniques` (many per question, written through `set_question_techniques()`), `lesson_techniques`; `curriculum_unit_steps.technique_ids` replaces the single pattern pin. Admin Techniques catalog at `/admin/techniques` (name, when to use it, process, default skills by name with all-Math / all-R&W shortcuts, inline create from the curriculum editor); technique tags on every review surface; the drill launcher draws technique-matching questions first and tops up from the skill. **B (2026-09-23):** `/tutor/tagging` + `/tutor/tagging/<unit>` — one question at a time with the answer in view, technique keys 1–9, Enter to save & next, Skip, Previous, N of M tagged, order switch; manager + admin (tutor tree, since the admin tree redirects managers). **C (2026-09-24):** `technique_source` on practice steps (default: the lesson just taught's techniques), `lesson_revision_techniques` through the draft workflow, a Techniques section on the lesson editor (admin + tutor drafts), a picker on the AI generate page, the generator writing the preceding lesson's techniques into each practice drill with a why-line that names them. **D (2026-09-24):** section foundation syllabi ("Before Math", "Before Reading & Writing") in the same table and editor, walked by the generator before a section's first task in any phase with completed lessons skipped, a rationale sentence, and the launcher's section fallback. **E (2026-09-24):** tutor "covered in session" — `lesson_progress.covered_by` / `covered_at` written only through `mark_lesson_covered()` / `unmark_lesson_covered()` (SECURITY DEFINER, `is_teacher()` + `can_view()`, a trigger keeps the columns staff-only; migration `20260924180000`), a Foundations card on the tutor's student page (where the student stands on each foundation lesson, Mark covered / Undo, who and when) and a roster Foundations column ("Covered" / "n of N") with a fewest-covered-first sort; nothing shows until a section syllabus has a published lesson, and neither waits for the syllabus switch. | Migration verified in dev (role gate, unknown-technique refusal, clear, delete-scrub trigger) and **applied to production 2026-09-23** (two pattern rows carried over; grants and gate re-probed live); catalog, editor and tagging screen walked in dev as admin. C's migration `20260924120000` and D's migration `20260924150000` applied to production 2026-09-24. E's migration `20260924180000` applied to dev 2026-09-24 and verified at the SQL level (role gate, roster gate through `can_view`, manager path, forge-proof trigger, both undo paths) and in the browser as the seeded teacher (mark → roster "Covered" → undo); **applied to production 2026-09-24** on the owner's go-ahead (tracked as version `20260924220628`; gate probes re-run live: no JWT, student, off-roster teacher, unknown student, unknown lesson all refused before any write; no rows carry a mark). Committed types unchanged: a regen from production differs from the dev-generated file only by the dev-only `student_has_lesson_assignment` function. |
