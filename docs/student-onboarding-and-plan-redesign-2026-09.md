...
| **2d · Techniques layer** — **schema + rename 2026-09-23** (step A of four) | Question patterns retired in favor of techniques (`foundations-and-question-patterns.md` §8): `techniques`, `technique_skills` (default applicability by skill), `question_techniques` (many per question, written through `set_question_techniques()`), `lesson_techniques`; `curriculum_unit_steps.technique_ids` replaces the single pattern pin. Admin Techniques catalog at `/admin/techniques` (name, when to use it, process, default skills by name with all-Math / all-R&W shortcuts, inline create from the curriculum editor); technique tags on every review surface; the drill launcher draws technique-matching questions first and tops up from the skill. Still to come: the per-unit tagging screen (B), technique pickers on lessons + the "questions for the lesson just taught" practice option (C), section foundations (D). | Migration verified in dev (role gate, unknown-technique refusal, clear, delete-scrub trigger); catalog and editor walked in dev as admin. Production migration pending the owner's go-ahead. |
