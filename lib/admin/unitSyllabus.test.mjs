...
test('a drill step: defaults, ranges, skill list, and technique narrowing', () => {
  ...
  assert.equal(narrowed.value.techniqueSource, 'explicit');
  // Blank source: a practice set follows its lesson; a mixed set never narrows.
  const none = normalizeStepInput({ kind: 'drill', techniqueIds: '' }, ctx);
  assert.ok(none.ok && none.value.techniqueIds === null && none.value.techniqueSource === 'lesson');
  assert.ok(mixed.ok && mixed.value.techniqueSource === 'none');
  const whole = normalizeStepInput({ kind: 'drill', techniqueSource: 'none' }, ctx);
  assert.ok(whole.ok && whole.value.techniqueSource === 'none' && whole.value.techniqueIds === null);
  // Explicit without a list is a mistake, not a silent widening.
  const emptyExplicit = normalizeStepInput({ kind: 'drill', techniqueSource: 'explicit' }, ctx);
  assert.ok(!emptyExplicit.ok && /at least one technique/.test(emptyExplicit.error));
  // A list under 'lesson' is dropped in favour of the lesson's own.
  const lessonWithList = normalizeStepInput({ kind: 'drill', techniqueSource: 'lesson', techniqueIds: [TECHNIQUES[0].id] }, ctx);
  assert.ok(lessonWithList.ok && lessonWithList.value.techniqueIds === null && lessonWithList.value.techniqueSource === 'lesson');
  const mixedExplicit = normalizeStepInput({ kind: 'drill', role: 'mixed', techniqueSource: 'explicit', techniqueIds: [TECHNIQUES[0].id] }, ctx);
  assert.ok(mixedExplicit.ok && mixedExplicit.value.techniqueSource === 'none' && mixedExplicit.value.techniqueIds === null);
  assert.ok(!normalizeStepInput({ kind: 'drill', techniqueSource: 'sometimes' }, ctx).ok);
