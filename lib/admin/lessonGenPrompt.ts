...
- \`video_placeholder\` — { kind, video_topic }: a precise description of a video an admin will source later. Use sparingly, where demonstration genuinely beats text and graphs. Never invent a URL. House convention: every lesson MAY open with a 60–90 second tutor intro video as block 1 (face + whiteboard: name the move, the test-day payoff, one caution) — when you include one, make it the FIRST block and write video_topic as that brief.
...
Solve every check and every Desmos validation yourself and confirm exactly one choice/expression is correct (for a numeric-entry check: that answer and every accept entry are the same value, typed the way a student would). Across the complete lesson, deliberately distribute correct answers among the available choice positions — no obvious pattern, correct_index updated after any reordering.
