...
import { SVGmsqrt } from 'mathjax-full/js/output/svg/Wrappers/msqrt.js';
import { SVGmroot } from 'mathjax-full/js/output/svg/Wrappers/mroot.js';

const adaptor = liteAdaptor();
RegisterHTMLHandler(adaptor);

// MathJax 3.2.x (both 3.2.1 and 3.2.2) mis-sizes an <mroot> whose
// index is wider than the surd's overlap allowance (three or more
// digits, e.g. the 144th root). SVGmroot.addRoot stores the surd's
// horizontal offset in `this.dx`, but SVGWrapper reserves `dx` for
// the node's CSS left padding and adds it again in place(), so the
// parent translates the whole root group right by that offset a
// second time while the viewBox is sized for a single shift — the
// last radicand glyph is cut off. Keep the surd offset in its own
// field so place() only applies real padding. Output is byte-identical
// for every root whose index fits (offset 0), so re-rendering is safe.
SVGmroot.prototype.addRoot = function addRoot(ROOT, root, sbox, H) {
  root.toSVG(ROOT);
...
SVGmsqrt.prototype.toSVG = function toSVG(parent) {
  ...
  const x = this.surdOffset;
  surd.toSVG(SVG);
  surd.place(x, H - sbox.h);
  base.toSVG(BASE);
  base.place(x + sbox.w, 0);
  this.adaptor.append(SVG, this.svg('rect', {
    width: this.fixed(bbox.w), height: this.fixed(t),
    x: this.fixed(x + sbox.w), y: this.fixed(H - t),
  }));
};
