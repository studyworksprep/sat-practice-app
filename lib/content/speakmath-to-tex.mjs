...
// Coordinates.
const RE_COORDS = /\bwith coordinates\s+(-?[A-Za-z0-9.]+(?:\s*,\s*-?[A-Za-z0-9.]+)+)/g;
...
function structuralPass(s) {
  ...
  do {
    ...
    cur = cur.replace(RE_COORDS, (_, body) => `\\left(${body.replace(/\s*,\s*/g, ', ')}\\right)`);
...
export function parseOrNull(alt) {
  ...
  const RESIDUAL = /\b(squared|cubed|superscript|numerator|denominator|parenthesis|bracket|end root|square root|cube root|absolute value|upper|power|raised|less than|greater than|approximately|divided by|plus|minus|times|equals|percent|degrees|negative)\b/i;
