...
// Bare YYYY-MM-DD is a calendar date with no time and no zone — e.g. the
// value of an <input type="date">, or a Postgres `date` column like
// test_registrations.test_date. `new Date("2026-06-06")` parses it as UTC
// midnight, then toLocaleDateString shifts it into the user's zone, which
// renders as the previous day for anyone west of UTC. Treat date-only
// inputs as local instead.
export function parseLocalOrIso(iso) {
...
/**
 * Is a calendar-date deadline (bare YYYY-MM-DD, e.g. from a `date`
 * column or an <input type="date">) already past?
 *
 * Treats the deadline as end-of-day in the viewer's local zone — so
 * an assignment "due May 24" stays not-overdue all day on May 24,
 * and flips to overdue at local midnight on May 25. That matches
 * the way students read "due May 24" intuitively, and avoids the
 * pre-fix bug where Date.parse("2026-05-24") < now flipped overdue
 * at midnight UTC (8pm the previous evening on the US East Coast).
 */
export function isPastDueDate(iso, nowMs = Date.now()) {
  if (!iso) return false;
  const d = parseLocalOrIso(iso);
  if (Number.isNaN(d.getTime())) return false;
  const DAY_MS = 24 * 60 * 60 * 1000;
  return d.getTime() + DAY_MS <= nowMs;
}
