...
export async function recomputeAttemptScores(supabase, attemptId) {
  ...
  //    the scoring ceiling). `attempted` flips true when at least
  //    one module attempt exists for the subject; an absent section
  //    (single-section attempt) stays `attempted: false` so we can
  //    leave its scaled score null instead of feeding (0,0) into the
  //    scaling helper.
  const bySubject = {
    RW:   { m1: 0, m2: 0, totalItems: 0, route: 'std', moduleIds: [], attempted: false },
    MATH: { m1: 0, m2: 0, totalItems: 0, route: 'std', moduleIds: [], attempted: false },
  };
  ...
  // 5. Compute scaled scores per section, then composite. Sections
  //    with no module attempts (single-section attempt) score as
  //    null; compositeScore() then returns null since one side is
  //    missing. This mirrors how the legacy tree represented
  //    section-only attempts before the metadata column was dropped.
  const rwScaled = bySubject.RW.attempted
    ? scaleSectionScoreWithLookup({
        subject: 'RW',
    ...
    : null;
  const mathScaled = bySubject.MATH.attempted
    ? scaleSectionScoreWithLookup({
        subject: 'MATH',
