...
export async function loadTestResults({ supabase, attemptId, viewerUserId, viewerRole }) {
  ...
  // Pattern catalog scoped to the skills on this test, so the picker
  // offers only patterns that could apply to each question.
  const questionPatternsCanTag = canTagPatterns(viewerRole);
  const questionPatternsCatalog = await loadPatternCatalog({
    role: viewerRole,
    skillCodes: reviewItems
