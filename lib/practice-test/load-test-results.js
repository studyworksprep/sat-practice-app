...
export async function loadTestResults({ supabase, attemptId, viewerUserId, viewerRole }) {
  ...
  const conceptTagsCanDelete = viewerRole === 'manager' || viewerRole === 'admin';
  ...
  // Technique catalog + each question's tags, for the picker on the
  // results page (manager/admin only).
  const techniquesCanTag = canTagTechniques(viewerRole);
  const techniqueCatalog = await loadTechniqueCatalog({ role: viewerRole });
  if (techniquesCanTag) {
    const techniquesByQid = await loadQuestionTechniqueIds({
      questionIds: reviewItems.filter((it) => !it.missing).map((it) => it.questionId),
    });
    for (const it of reviewItems) {
