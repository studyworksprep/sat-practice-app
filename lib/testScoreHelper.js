...
export async function computeTestScores(supabase, completedAttempts) {
  ...
  const { data: tests } = await supabase.from('practice_tests_v2').select('id, name').in('id', testIds);
  const testNameById = {};
  ...
  const { data: mods } = modIds.length
    ? await supabase.from('practice_test_modules_v2').select('id, subject_code, module_number, route_code').in('id', modIds)
    : { data: [] };
