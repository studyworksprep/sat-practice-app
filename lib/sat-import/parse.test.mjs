...
test('standalone equations retain compact size and centering after rendering and sanitization', async () => {
 ...
 // MathJax emits height="2.782ex"; sanitizeQuestionHtml rewrites the
 // svg sizing to em (lib/content/math-svg-units.ts): 2.782 × 0.5461.
 assert.match(html, /height="1.519em"/);
 assert.doesNotMatch(html, /height="[\d.]+ex"/);
...
test('merged table headings retain spans and alignment through sanitization', async () => {
 const {sanitizeQuestionHtml}=await import('../sanitize.ts');
 const html=sanitizeQuestionHtml(mmdToHtml(String.raw`\begin{tabular}{|l|l|l|l|}
\hline \multirow[b]{2}{*}{Group} & \multicolumn{3}{|c|}{Diameter (millimeters)} \\
\hline & Less than 20 & 20 to 30 & Greater than 30 \\
\hline Group 1 & 12 & 8 & 3 \\
\end{tabular}`));
 assert.match(html, /rowspan="2">Group<\/th>/);
 assert.match(html, /colspan="3">Diameter \(millimeters\)<\/th>/);
 assert.equal((html.match(/<th /g)??[]).length,5);
 assert.equal((html.match(/<td /g)??[]).length,4);
 assert.ok(!html.includes('multirow')&&!html.includes('multicolumn'));
 assert.throws(()=>mmdToHtml(String.raw`\begin{tabular}{l}\multicolumn{0}{c}{bad}\end{tabular}`),/Invalid table/);
});

test('prose percentages render literally while TeX percent escapes remain in math', async()=>{
 const {renderHtml}=await import('../content/render-math.mjs');
 const {sanitizeQuestionHtml}=await import('../sanitize.ts');
 const html=mmdToHtml(String.raw`24\% and 40\%, or $32 \%$.`);
 assert.ok(html.includes('24% and 40%'));
 assert.ok(html.includes(String.raw`\(32 \%\)`));
 const clean=sanitizeQuestionHtml(renderHtml(html)||html);
 assert.ok(!clean.includes(String.raw`\%`));
 assert.match(mmdToHtml(String.raw`\begin{tabular}{ll}Capacity & Proportion \\ Less than 18 & 26\% \\\end{tabular}`),/>26%<\/td>/);
});
