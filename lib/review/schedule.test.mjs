...
// ── Lesson retrieval (plan 5.3) ────────────────────────────────────

test('a finished lesson comes due two days later', () => {
  assert.equal(LESSON_RETRIEVAL_DELAY_DAYS, 2);
  assert.equal(
    lessonRetrievalDueIso('2026-07-17T12:00:00.000Z'),
    '2026-07-19T12:00:00.000Z',
  );
});

test('lesson retrieval due date is not due yet at completion time', () => {
  const completedAt = '2026-07-17T12:00:00.000Z';
  const due = lessonRetrievalDueIso(completedAt);
  assert.equal(isDue(due, completedAt), false);
  // ...and is due once two days have passed.
  assert.equal(isDue(due, '2026-07-19T12:00:00.000Z'), true);
});

test('lesson retrieval accepts an offset-format completion stamp', () => {
  assert.equal(
    lessonRetrievalDueIso('2026-07-17T12:00:00+00:00'),
    '2026-07-19T12:00:00.000Z',
  );
});
