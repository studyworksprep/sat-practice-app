...
const getUserAndProfile = cache(async () => {
  ...
  //
  // is_demo gates writes for the marketing demo accounts. The DB
  // is the authoritative layer (restrictive policies deny INSERT/
  // UPDATE/DELETE for is_demo JWTs); this field exists so server
  // routes can fail fast with a clear 403 via requireWriter()
  // instead of forwarding the request and surfacing a Postgres
  // RLS error to the user.
  const { data: profile, error: profileErr } = await supabase
    .from('profiles')
    .select('id, role, subscription_exempt, ui_version, is_demo, first_name')
    .eq('id', user.id)
    .maybeSingle();
  ...
  return {
    user,
    profile: profile ?? { id: user.id, role: 'practice', is_demo: false, first_name: null },
    supabase,
  };
});
