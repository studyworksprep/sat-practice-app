...
test('isActive: the session runner lights up the Practice tab', () => {
  const practice = STUDENT_LINKS.find((l) => l.label === 'Practice Questions');
  assert.ok(practice);
...
test('Dashboard anchors the student sidebar; Plan joins it only with an active plan', () => {
  const withoutPlan = hrefs(flatLinks(studentSections()));
  assert.equal(withoutPlan[0], '/dashboard', 'Dashboard is the first link');
  assert.ok(!withoutPlan.includes('/plan'), 'no Plan link without a plan');
  assert.ok(!withoutPlan.includes('/today'), 'Today is no longer a page');
  assert.ok(withoutPlan.includes('/performance'), 'Performance is always on-nav');

  const withPlan = hrefs(flatLinks(studentSections({ hasPlan: true })));
  assert.equal(withPlan[0], '/dashboard', 'Dashboard stays first with a plan');
  assert.equal(withPlan[1], '/plan', 'the plan hub sits right after Dashboard');
  assert.equal(withPlan[2], '/performance', 'Performance closes the anchor group');
  assert.deepEqual(withPlan.filter((h) => h !== '/plan'), withoutPlan);
});

test('tutor sections cover every tutor top-nav link, lesson library, and lesson packs', () => {
  for (const role of ['teacher', 'manager']) {
    ...
    assert.ok(sectionHrefs.has('/tutor/lessons'), `${role}: lesson library on-nav`);
  }
});
...
test('live runner surfaces suppress the shell', () => {
  assert.equal(isShellSuppressedPath('/welcome'), true, 'the intake renders bare');
