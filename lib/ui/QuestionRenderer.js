...
// ---------- Draggable pane divider (two-column layout) ----------
...
export function QuestionRenderer({
  ...
  // Pane split for the two-column layout. Hooks stay unconditional
  // (the two-column path is a conditional return below); the state
  // is simply unused elsewhere. Restore runs post-mount so server
  // + first client render agree — same pattern as the Desmos
  // toggle in the runners.
  ...
  function commitPaneSplit(pct) {
    if (pct == null) {
      ...
      setPaneSplit(null);
      try { window.localStorage.removeItem(PANE_SPLIT_KEY); } catch { /* ignore */ }
      return;
    }
    const next = clampSplitPct(pct);
    setPaneSplit(next);
    try { window.localStorage.setItem(PANE_SPLIT_KEY, String(Math.round(next))); } catch { /* ignore */ }
  }

  // Pointer capture routes every move to the divider (the drag
  // survives crossing the injected passage HTML), and the var is
  // written directly during the drag — React state commits once,
  // on release (presenter mode's divider pattern: smoother than a
  ...
  if (leftSlot != null) {
    ...
    return (
      <div ref={typesetRef} className={s.typesetRoot}>
        {taxonomyNode}
        <div className={slotClasses} data-subject={subjectAttr}>
          <div
