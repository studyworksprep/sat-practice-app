...
// ?debug=layout on any URL toggles colored outlines + labels on
// every container so you can see exactly which element is
// collapsing. Opt-in via query param so a live page is never
// affected unless you specifically ask for it.
...
function mergeDebug(base, debug, label, color) {
  ...
  return {
    ...
    outlineOffset: -2,
    position: 'relative',
    // Label injected via ::before is out of reach for inline
    // styles; we use an attribute + a global CSS attr selector
    // set up in app/globals.css. For now, just the outline; the
    // element's tag + computed width is enough to identify it in
    // DevTools once the outline is visible.
  };
}
 ...
 *   Optional left-pane content (e.g. a DesmosPanel for math
 *   questions). Overrides the stimulus-based two-column path: when
 *   leftSlot is provided, two-column is always used and the left
 *   pane renders leftSlot instead of stimulus. Stimulus still
 *   appears at the top of the right pane in that case.
 * @param {boolean} [props.leftSlotCollapsed]
 *   When true, the left pane's grid track collapses to 0 (with a
 *   CSS transition) so the left content is hidden but stays
 *   mounted — lets callers like the Desmos toggle animate the
 *   panel in and out without remounting its internal state.
 */
export function QuestionRenderer({
  ...
  // ?debug=layout toggles colored outlines + labels on every
  // container so we can see exactly which element is collapsing.
  // Persistent between renders via useState + window.location.
  ...
  // Three paths to the two-column card:
  //   1. leftSlot provided — use it as left pane (Desmos, etc).
  //      Stimulus moves to the top of the right pane (single-col).
  //   2. layout='two-column' + stimulus, wide viewport — use
  //      stimulus as left pane (reading questions).
  //   3. Anything else — single-column stack.
  //
  // Path 1 also supports a "collapsed" state where the left track
  // animates to 0 width so the caller can hide the slot without
  // remounting its content (critical for DesmosPanel so the
  // calculator's internal state survives a toggle).
  const useSlotLayout = leftSlot != null && isWide;

  if (useSlotLayout) {
    const gridStyle = {
      ...S.twoCol,
      gridTemplateColumns: leftSlotCollapsed ? '0fr 1fr' : '1fr 1fr',
      transition: 'grid-template-columns 220ms cubic-bezier(0.4, 0, 0.2, 1)',
    };
    const leftStyle = {
      ...S.twoColLeft,
      // Remove the padding / divider when the pane is collapsed
      // so nothing visible bleeds out of the zero-width track.
      padding: leftSlotCollapsed ? 0 : S.twoColLeft.padding,
      borderRight: leftSlotCollapsed ? 'none' : S.twoColLeft.borderRight,
      overflow: 'hidden',
    };
    return (
      <>
        {taxonomyNode}
        <div style={mergeDebug(gridStyle, debug, 'twoCol (slot)', '#e11d48')}>
          <div style={mergeDebug(leftStyle, debug, 'leftSlot', '#2563eb')}>
            {leftSlot}
          </div>
          <div style={mergeDebug(S.twoColRight, debug, 'twoColRight', '#16a34a')}>
            {stimulusNode}
            {stemNode}
            {responseNode}
            {resultNode}
  ...
  if (useTwoColumn && stimulusNode) {
    return (
      <>
        {taxonomyNode}
        <div style={mergeDebug(S.twoCol, debug, 'twoCol', '#e11d48')}>
          <div style={mergeDebug(S.twoColLeft, debug, 'twoColLeft', '#2563eb')}>
            {stimulusNode}
          </div>
          <div style={mergeDebug(S.twoColRight, debug, 'twoColRight', '#16a34a')}>
            {stemNode}
...
const S = {
  ...
  // reads at a glance.
  // Two-column card. Previously used `1fr 1px 1fr` expecting a
  // separate divider element in the grid, but the JSX only
  // renders two children — so the right pane landed in the 1px
  // column and collapsed. Simpler: plain two-column grid with a
  // right border on the left pane acting as the visual divider.
  twoCol: {
    display: 'grid',
  ...
  optionsFieldset: {
    ...
    // <fieldset> defaults to min-inline-size: min-content, which
    // shrinks the element to the width of its smallest unbreakable
    // word. Override to 0 so the fieldset fills its parent flex
    // column instead of collapsing.
    minInlineSize: 0,
    width: '100%',
  },
  srOnly: {
