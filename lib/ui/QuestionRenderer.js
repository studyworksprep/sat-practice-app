...
// ---------- Draggable pane divider (two-column layouts) ----------
// Bluebook's reading view lets students drag the boundary between
// the passage and question panes; this ports presenter mode's
// divider (pointer capture, arrow keys, persisted width) to the
// shared renderer so the runners and report surfaces all get the
// same control. The chosen split rides a CSS var on the grid —
// null state = no var = the stylesheet defaults (even split, or
// the wider 6.5b passage pane when the stimulus carries a table).
//
// The Desmos slot layout gets the same divider under its own
// storage key — calculator width and passage width are separate
// preferences (a student may want a slim calculator but a wide
// passage). Slot-layout tracks are expressed in %, not fr, so the
// calculator's open/collapse grid transition still interpolates.
const PANE_SPLIT_KEY = 'sw:qr-pane-split';
const CALC_SPLIT_KEY = 'sw:qr-calc-split';
const SPLIT_MIN_PCT = 25;
const SPLIT_MAX_PCT = 70;
const clampSplitPct = (p) => Math.min(SPLIT_MAX_PCT, Math.max(SPLIT_MIN_PCT, p));
...
const HtmlBlock = memo(function HtmlBlock({ as = 'span', className, html }) {
  const Tag = as;
  // Defense-in-depth — admin-authored content goes through the
  // sanitizer before it hits dangerouslySetInnerHTML. The
  // question profile preserves all the math markup we need;
  // <script>, on*, and javascript: URIs are scrubbed.
  const safe = useMemo(() => sanitizeQuestionHtml(html ?? ''), [html]);
  return (
    <Tag
      className={className}
...
// MathJax typeset + ACT qref highlight effects live in
// lib/ui/preview-effects.js so the ACT-import review surface
// can drive the same rendering passes over admin-authored
// HTML without duplicating the implementation.
 ...
 *   stimulus and stem.
 * @param {'rw'|'math'|null} [props.subject] — subject identity for the
 *   card's top rail color; omit for a neutral card.
 * @param {boolean} [props.frameless]
 */
export function QuestionRenderer({
  ...
  onToggleCross,
  /** Subject identity ('rw' | 'math' | null). Drives the top
   *  rail color on the question card via [data-subject]. Pages
   *  that don't pass a subject get a neutral card (no rail). */
  subject = null,
  /** Presenter variant: drops the rounded-card chrome so the
   *  two-column layouts fill the host edge-to-edge (parity with
   *  the presenter's Desmos split), and clamps the single-column
   *  stack to a readable width instead. */
  frameless = false,
}) {
  ...
  useQrefHighlight(typesetRef, question.qrefOrdinal ?? null, question.stimulusHtml);

  // Pane split for the two-column layouts. Hooks stay unconditional
  // (both two-column paths are conditional returns below); the
  // state is simply unused elsewhere. The slot (calculator) layout
  // and the reading layout persist under separate keys. Restore
  // runs post-mount so server + first client render agree — same
  // pattern as the Desmos toggle in the runners.
  const splitKey = leftSlot != null ? CALC_SPLIT_KEY : PANE_SPLIT_KEY;
  const twoColRef = useRef(null);
  const [paneSplit, setPaneSplit] = useState(null);
  useEffect(() => {
    try {
    ...
    } catch { /* storage unavailable — keep the stylesheet defaults */ }
  }, [splitKey]);

  function commitPaneSplit(pct) {
    if (pct == null) {
      // Reset (double-click): back to the stylesheet defaults.
      setPaneSplit(null);
      try { window.localStorage.removeItem(splitKey); } catch { /* ignore */ }
      return;
    }
    const next = clampSplitPct(pct);
    setPaneSplit(next);
    try { window.localStorage.setItem(splitKey, String(Math.round(next))); } catch { /* ignore */ }
  }

  // Pointer capture routes every move to the divider (the drag
  // survives crossing the injected passage HTML / Desmos canvas),
  // and the var is written directly during the drag — React state
  // commits once, on release (presenter mode's divider pattern:
  // smoother than a re-render per pointermove). The grid's
  // transition is suspended for the drag's duration: the slot
  // layout animates its open/collapse track change, and easing
  // every pointermove would rubber-band the divider.
  function onDividerPointerDown(e) {
    if (e.button !== 0) return;
    const grid = twoColRef.current;
    if (!grid) return;
    const rect = grid.getBoundingClientRect();
    const divider = e.currentTarget;
    const prevTransition = grid.style.transition;
    grid.style.transition = 'none';
    const pctAt = (clientX) => clampSplitPct(((clientX - rect.left) / rect.width) * 100);
    const onMove = (ev) => {
      grid.style.setProperty('--sw-pane-split', `${pctAt(ev.clientX)}%`);
    };
    const finish = (ev) => {
      grid.style.transition = prevTransition;
      commitPaneSplit(pctAt(ev.clientX));
  ...
  const subjectAttr = subject ?? undefined;
  ...
  // The divider stays mounted while collapsed (zero-width, inert)
  // so the grid keeps three tracks in both states and the
  // open/collapse transition still interpolates.
  if (leftSlot != null) {
    const slotClasses = [
      s.twoCol,
    ...
    const effectiveSplit = paneSplit ?? 50;
    return (
      <div ref={typesetRef} className={frameless ? `${s.typesetRoot} ${s.frameless}` : s.typesetRoot}>
        {taxonomyNode}
        <div
          ref={twoColRef}
          className={slotClasses}
          data-subject={subjectAttr}
          style={paneSplit != null ? { '--sw-pane-split': `${paneSplit}%` } : undefined}
        >
          <div
            className={leftSlotCollapsed ? s.slotPaneCollapsed : `${s.twoColLeft} ${s.slotPane}`}
          >
            {leftSlot}
          </div>
          <div
            className={`${s.paneDivider} ${leftSlotCollapsed ? s.paneDividerCollapsed : ''}`}
            role="separator"
            aria-orientation="vertical"
            aria-label="Resize calculator pane (drag, or left and right arrow keys; double-click to reset)"
            aria-valuemin={SPLIT_MIN_PCT}
            aria-valuemax={SPLIT_MAX_PCT}
            aria-valuenow={Math.round(effectiveSplit)}
            aria-hidden={leftSlotCollapsed || undefined}
            tabIndex={leftSlotCollapsed ? -1 : 0}
            onPointerDown={leftSlotCollapsed ? undefined : onDividerPointerDown}
            onKeyDown={(e) => {
              if (leftSlotCollapsed) return;
              if (e.key !== 'ArrowLeft' && e.key !== 'ArrowRight') return;
              e.preventDefault();
              commitPaneSplit(effectiveSplit + (e.key === 'ArrowRight' ? 4 : -4));
            }}
            onDoubleClick={leftSlotCollapsed ? undefined : () => commitPaneSplit(null)}
          />
  ...
  if (layout === 'two-column' && stimulusNode) {
    // Data-bearing stimuli get a wider passage pane (see
    // .twoCol[data-stimulus-table] in the module CSS) so multi-
    // column tables fit without the overflow-x scrollbar.
    const stimulusHasTable =
      typeof question.stimulusHtml === 'string'
      && question.stimulusHtml.includes('<table');
    // Baseline for the keyboard step / aria-valuenow when the
    // student hasn't dragged yet — mirrors the stylesheet default
    // for this question shape.
    const effectiveSplit = paneSplit ?? (stimulusHasTable ? 60 : 50);
    return (
      <div ref={typesetRef} className={frameless ? `${s.typesetRoot} ${s.frameless}` : s.typesetRoot}>
        {taxonomyNode}
        <div
          ref={twoColRef}
          className={`${s.twoCol} ${s.twoColSplit}`}
          data-subject={subjectAttr}
          data-stimulus-table={stimulusHasTable ? '' : undefined}
          style={paneSplit != null ? { '--sw-pane-split': `${paneSplit}%` } : undefined}
        >
          <div className={s.twoColLeft}>{stimulusNode}</div>
          <div
            className={s.paneDivider}
            role="separator"
            aria-orientation="vertical"
            aria-label="Resize passage pane (drag, or left and right arrow keys; double-click to reset)"
            aria-valuemin={SPLIT_MIN_PCT}
            aria-valuemax={SPLIT_MAX_PCT}
            aria-valuenow={Math.round(effectiveSplit)}
            tabIndex={0}
            ...
            onDoubleClick={() => commitPaneSplit(null)}
          />
          <div className={s.twoColRight}>
            {headerNode}
            {stemNode}
  ...
  // Path 3: single-column stack. In frameless mode the stack gets a
  // readable-width clamp — the host is full-bleed there, and answer
  // options spanning a projector are unreadable.
  const singleStack = (
    <>
      {headerNode}
      {stimulusNode}
  ...
  return (
    <div ref={typesetRef} className={frameless ? `${s.typesetRoot} ${s.frameless}` : s.typesetRoot}>
      {taxonomyNode}
      {frameless ? <div className={s.framelessSingle}>{singleStack}</div> : singleStack}
    </div>
  );
}
