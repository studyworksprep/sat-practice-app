...
const S = {
  ...
  // reads at a glance.
  // Matches the design kit's .qaTwoCol grid exactly — 1fr, 1px
  // divider, 1fr. The panes take equal space; content wraps inside
  // each as needed. width:100% forces the grid to fill its flex
  // parent's cross-axis so it doesn't shrink to its content's
  // min-content (which would happen if e.g. a child fieldset
  // shrinks to its smallest word).
  twoCol: {
    display: 'grid',
