...
// ACT qref highlight. The English + Reading parsers emit
// `<span class="qref" data-q="N">…</span>` markers around the
// underlined portion (English) or line-referenced span (Reading)
// each question targets. The renderer scopes a query to the
// typeset root, finds the marker that matches the active
// question's ordinal, and toggles `.qref-active` so a CSS rule
// can highlight it. SAT questions carry qrefOrdinal=null and
// the effect short-circuits.
function useQrefHighlight(ref, qrefOrdinal, stimulusKey) {
  useEffect(() => {
  ...
  // stimulusKey re-runs the effect when the rendered HTML
  // changes (e.g. navigating to a new question in the same
  // passage).
  }, [ref, qrefOrdinal, stimulusKey]);
}
...
export function QuestionRenderer({
   ...
   *  rail color on the question card, the cream wash on the
   *  reading-passage column, and the soft math-blue wash on
   *  the Desmos slot pane via [data-subject] / [data-passage] /
   *  [data-pane]. Pages that don't pass a subject get the
   *  pre-visual-pass look (no rail, white columns). */
  subject = null,
}) {
  ...
  if (layout === 'two-column' && stimulusNode) {
    return (
      <div ref={typesetRef} className={s.typesetRoot}>
        {taxonomyNode}
        <div className={s.twoCol} data-subject={subjectAttr}>
          <div className={s.twoColLeft} data-passage="paper">{stimulusNode}</div>
          <div className={s.twoColRight}>
            {headerNode}
            {stemNode}
            {responseNode}
