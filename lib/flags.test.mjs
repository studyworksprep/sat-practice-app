...
import { entitlementsGateEnabled, resolveSidebarStage } from './flags.ts';
...
test('entitlements gate: exactly "on" enables; everything else is legacy', () => {
  assert.equal(entitlementsGateEnabled('on'), true);
  for (const value of ['off', null, undefined, '', 'ON', 'true', '1', 'enabled']) {
    assert.equal(entitlementsGateEnabled(value), false, String(value));
  }
});
