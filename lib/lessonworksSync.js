...
export async function buildStudentPayload(supabase, studentId, { includePdfReports, testLimit }) {
  ...
  const totalQuestionsDone = doneCount ?? 0;
  const practiceStats = computePracticeStats(practiceAttempts, assignStudentRows || [], supabase, studentId, totalQuestionsDone);

  // ── Taxonomy map for mastery ──
  //
  // attempts.question_id carries either a v2 UUID (any attempt that
  // came through the new tree after migration 20240101000017 dropped
  // the v1 FK) or a v1 UUID (legacy-era attempts). We have to look
  // both up so a student whose practice straddled the v2 cutover
  // doesn't end up with most of her work bucketed into a phantom
  // "Unknown" domain. v2 data wins on overlap because it's the
  // canonical taxonomy after the migration.
  ...
  const taxMap = {};
  if (allQids.length > 0) {
    const [{ data: v2Rows }, { data: v1Rows }] = await Promise.all([
      supabase
        .from('questions_v2')
    ...
    // v1 first, then v2 overwrites — preserves the v2-wins ordering.
    for (const t of (v1Rows || [])) taxMap[t.question_id] = t;
 ...
 */
async function computePracticeStats(practiceAttempts, assignStudentRows, supabase, studentId, totalQuestionsDone) {
