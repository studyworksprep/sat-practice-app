...
test('a pinned pattern rides the drill\'s filter_criteria', () => {
  const steps = syllabi();
  steps['H.A.'][1].patternId = 'pattern-1';
  const plan = generatePlan(fixture({ unitSteps: steps }));
  const practice = skillTasks(plan)[1];
  assert.equal(practice.payload.filter_criteria.pattern_id, 'pattern-1');
});
...
test('buildSyllabi groups by skill, orders by position, drops unpublished lessons', () => {
  const rows = [
    { id: 'b', position: 2, kind: 'drill', lesson_id: null, role: 'practice', skill_codes: null, pattern_id: null, question_count: 8, minutes: null, skip_if_completed: true, unit: { skill_code: 'H.A.', test_type: 'sat' }, lesson: null },
    { id: 'a', position: 1, kind: 'lesson', lesson_id: 'L1', role: null, skill_codes: null, pattern_id: null, question_count: null, minutes: null, skip_if_completed: true, unit: { skill_code: 'H.A.', test_type: 'sat' }, lesson: { title: 'Graphing', status: 'published' } },
    { id: 'c', position: 3, kind: 'lesson', lesson_id: 'L2', role: null, skill_codes: null, pattern_id: null, question_count: null, minutes: null, skip_if_completed: true, unit: { skill_code: 'H.A.', test_type: 'sat' }, lesson: { title: 'Draft', status: 'draft' } },
    { id: 'd', position: 1, kind: 'drill', lesson_id: null, role: 'mixed', skill_codes: ['H.A.', 'H.B.'], pattern_id: null, question_count: null, minutes: 25, skip_if_completed: true, unit: [{ skill_code: 'H.B.', test_type: 'sat' }], lesson: null },
  ];
