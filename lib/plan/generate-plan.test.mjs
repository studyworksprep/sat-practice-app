...
import { generatePlan, buildDrillPayload } from './generate-plan.ts';
...
test('no task is scheduled after the test date (non-week-aligned horizon)', () => {
  const f = fixture();
  f.testDate = '2026-10-15'; // 75 days after today → 11 weeks, not a clean multiple
  const plan = generatePlan(f);
  assert.ok(plan.tasks.length > 0);
  for (const t of plan.tasks) {
    assert.ok(t.scheduledDate <= f.testDate, `${t.scheduledDate} is after test date ${f.testDate}`);
    assert.ok(t.scheduledDate >= f.today, `${t.scheduledDate} before today`);
  }
});
...
// ── Why-this reasons ─────────────────────────────────────────────
//
// The generator stores a CAUSE code, never a point shortfall: mastery is
// damped by a volume factor, so "N points below the bar" mostly reported
// how little a student had practiced while reading as a verdict on how
// they were doing.

/** The first drill for a lone skill in the given state. */
function drillFor(skill) {
  const plan = generatePlan({
    ...fixture(),
    skills: [{
      domainCode: 'CAS', skillCode: 'WIC', section: 'reading_writing',
      masteryThreshold: 80, learnability: 6, expectedMinutes: 45, sequence: 1,
      questionsAvailable: 200, hasLesson: false, coverageStatus: 'practiced',
      ...skill,
    }],
  });
  const drill = plan.tasks.find((t) => t.taskType === 'drill');
...
test('no generated task ever mentions headroom or a point shortfall', () => {
  const plan = generatePlan(fixture());
  for (const t of plan.tasks) {
    assert.doesNotMatch(String(t.payload.why ?? ''), /headroom/i, `${t.taskType} leaks headroom`);
  }
});

test('drills carry a reason code and the attempt count behind it', () => {
  const plan = generatePlan(fixture());
  for (const t of plan.tasks.filter((x) => x.taskType === 'drill')) {
    assert.ok(typeof t.payload.why_code === 'string', 'drill has no why_code');
    assert.ok(typeof t.payload.why_attempts === 'number', 'drill has no why_attempts');
    assert.ok(String(t.payload.why ?? '').length > 0, 'drill has no rendered why');
  }
});

test('too few attempts → the copy is about evidence, not shortfall', () => {
  // 5 questions at 100% still scores ~53 of 100 — the score is reporting
  // sample size, so the sentence must too.
  const p = drillFor({ mastery: 53, attemptsCount: 5 });
  assert.equal(p.why_code, 'low_evidence');
  assert.equal(p.why_attempts, 5);
  assert.match(p.why, /Only 5 questions so far/);
});

test('enough attempts → the band follows implied accuracy', () => {
  assert.equal(drillFor({ mastery: 78, attemptsCount: 15 }).why_code, 'near_threshold');
  assert.equal(drillFor({ mastery: 70, attemptsCount: 15 }).why_code, 'mixed');
  assert.equal(drillFor({ mastery: 55, attemptsCount: 20 }).why_code, 'shaky');
});

test('coverage status still wins over the accuracy bands', () => {
  assert.equal(drillFor({ mastery: null, attemptsCount: 0, coverageStatus: 'not_started' }).why_code, 'not_started');
  assert.equal(drillFor({ mastery: 55, attemptsCount: 40, coverageStatus: 'decayed' }).why_code, 'decayed');
});

test('a hand-written why (tutor add / skill swap) is stored verbatim, uncoded', () => {
  const p = buildDrillPayload({ domainCode: 'H', skillCode: 'H.A.' }, 'Added by your tutor');
  assert.equal(p.why, 'Added by your tutor');
  assert.equal('why_code' in p, false);
});
