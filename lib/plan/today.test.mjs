...
test('ahead: the next few future pending tasks, startable now', () => {
  const tasks = [
    task({ id: 'a', scheduledDate: '2026-07-16' }),
    task({ id: 'b', scheduledDate: '2026-07-17' }),
    task({ id: 'c', scheduledDate: '2026-07-18' }),
    task({ id: 'd', scheduledDate: '2026-07-19' }),
    task({ id: 'done', scheduledDate: '2026-07-16', status: 'completed', completedAt: '2026-07-10T10:00:00Z' }),
  ];
  const view = buildTodayView(tasks, TODAY, null);
  assert.deepEqual(view.due, []);
  assert.deepEqual(view.ahead.map((t) => t.id), ['a', 'b', 'c']);
  assert.equal(view.upNext.id, 'a');
});
