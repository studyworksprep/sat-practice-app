...
export function generateScoreReportPdf(data, options = {}) {
  ...
  sectionEntries.forEach(([subj, sec], i) => {
    ...
    // Adaptive route — second module the student was placed on.
    // Hidden for non-adaptive tests (route_code='std').
    if (sec.routeCode === 'hard' || sec.routeCode === 'easy') {
      const routeLabel = sec.routeCode === 'hard' ? 'Hard' : 'Easy';
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(sec.routeCode === 'hard' ? 185 : 22, sec.routeCode === 'hard' ? 28 : 101, sec.routeCode === 'hard' ? 28 : 52);
      doc.text(`Module 2 · ${routeLabel}`, cx, scoreSubY + 10, { align: 'center' });
  ...
  for (const subj of SUBJECT_ORDER) {
    for (const modNum of [1, 2]) {
      ...
      const rows = qs.map(q => {
        ...
        return [String(q.module_position ?? q.ordinal), q.domain_name || '—', q.skill_name || '—', q.score_band != null ? `Band ${q.score_band}` : '—', fmtTime(q.time_spent_ms), yourAns, correctAns];
      });
      doc.autoTable({
        startY: y, margin: { left: marginL, right: marginR },
        head: [['Q#', 'Domain', 'Skill', 'Score Band', 'Time', 'Your Answer', 'Correct']],
        body: rows,
