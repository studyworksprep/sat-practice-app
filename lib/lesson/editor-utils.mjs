...
export function createStarterBlock(type = 'text', index = 0) {
  ...
  if (type === 'question_link') {
    return {
      id: `new_question_link_block_${suffix}`,
  ...
  if (type === 'lesson_complete') {
    return {
      id: `new_lesson_complete_block_${suffix}`,
...
export function duplicateBlock(block, index = 0) {
  ...
  // A duplicated revision block is new content, not another edit of the
  // canonical source row. Keeping this provenance would make promotion try to
  // publish two blocks with the same canonical id.
  delete copy.source_block_id;
...
export function getBlockLabel(block) {
  ...
  if (block.block_type === 'question_link') return block.content?.question_id || 'Question link';
  if (block.block_type === 'lesson_complete') return 'Lesson completion';
