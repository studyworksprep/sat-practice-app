...
import { validateLessonFigure } from './figure.mjs';
import { validateLessonContext } from './context.mjs';
import { isNumericCheck, validateNumericCheckContent } from './numeric-check.mjs';
...
export function validateLessonBlocks(blocks = []) {
  ...
  // A branch target may reference either a block's top-level id or its
  // content.id — the runtime resolves both (content.id is the identity
  // that survives the save→reload cycle). Accept both here so a valid
  // branching lesson isn't rejected at import.
  const idSet = new Set();
  list.forEach((b, i) => {
    idSet.add(blockIdOf(b, i));
  ...
  list.forEach((block, index) => {
    ...
    const calculator = block?.content?.calculator;
    for (const message of validateLessonCalculatorPresentation(calculator)) {
      add(makeIssue('error', 'calculator_presentation_invalid', message, {
        blockId,
        path: 'content.calculator',
        suggestion: 'Use display hidden/available/open and mode scratch/preset with valid optional fields.',
      }));
    }
    if (
      calculator?.mode === 'preset' &&
      !calculator?.initial_state &&
      !(Array.isArray(calculator?.initial_expressions) && calculator.initial_expressions.length > 0)
    ) {
      add(makeIssue('warning', 'calculator_preset_empty', 'Preset calculator has no starting graph.', {
        blockId,
        path: 'content.calculator',
        suggestion: 'Capture a starting graph or provide calculator.initial_expressions.',
      }));
    }

    for (const message of validateLessonFigure(block?.content?.figure)) {
      add(makeIssue('error', 'figure_invalid', message, {
        blockId,
        path: 'content.figure',
        suggestion: 'Provide figure.src and figure.alt strings (caption optional), or remove figure.',
      }));
    }

    for (const message of validateLessonContext(block?.content?.context)) {
      add(makeIssue('error', 'context_invalid', message, {
        blockId,
        path: 'content.context',
        suggestion: 'Provide context.html as a non-empty string (label optional), or remove context.',
      }));
    }

    if (block.block_type === 'check') {
      const content = block?.content || {};
      // Answer surface: a check is multiple-choice or numeric-entry
      // (plan 1.6), never both. Until 1.6 nothing validated the
      // choice shape at all — a check with one choice or a
      // correct_index past the end reached the runtime silently.
      if (isNumericCheck(content)) {
        for (const message of validateNumericCheckContent(content)) {
          add(makeIssue('error', 'check_numeric_invalid', message, {
            blockId,
            path: 'content',
            suggestion: 'For input: "numeric", give answer (a whole number, decimal, or a/b fraction), optional accept (equivalent typeable forms), optional tolerance ≥ 0, and no choices/correct_index.',
          }));
        }
      } else {
        ...
        const choicesOk =
          ...
          choices.length >= 2 &&
          choices.every((c) => (typeof c === 'string' && c.trim() !== '') || typeof c === 'number');
        if (!choicesOk) {
          add(makeIssue('error', 'check_choices_invalid', 'choices must be an array of at least 2 non-empty entries.', {
            blockId,
            path: 'content.choices',
            suggestion: 'Give 2–5 answer choices, or set input: "numeric" for a typed answer.',
          }));
        } else if (content.correct_index == null) {
          // The runtime defaults a missing index to 0; keep that
          // lenient, but say so — it is usually an authoring slip.
          add(makeIssue('warning', 'check_correct_index_missing', 'correct_index is missing; the runtime will treat choice A as correct.', {
            blockId,
            path: 'content.correct_index',
            suggestion: 'Set correct_index explicitly (0-based).',
          }));
        } else if (
          !Number.isInteger(content.correct_index) ||
          content.correct_index < 0 ||
          content.correct_index >= choices.length
        ) {
          add(makeIssue('error', 'check_correct_index_invalid', `correct_index must be an integer from 0 to ${choices.length - 1}.`, {
            blockId,
            path: 'content.correct_index',
            suggestion: 'Point correct_index at the keyed choice (0-based).',
          }));
        }
      }
      const solution = content.solution;
      if (solution != null && (typeof solution !== 'string' || solution.trim() === '')) {
        add(makeIssue('error', 'check_solution_invalid', 'solution must be a non-empty string when present.', {
          blockId,
          path: 'content.solution',
          suggestion: 'Write the worked solution as plain text + LaTeX (newlines separate steps), or remove the key.',
        }));
      }
      if (typeof solution === 'string' && solution.trim() !== '' && !block?.content?.allow_retry) {
        add(makeIssue('warning', 'check_solution_without_retry', 'solution is set on a one-shot check, where it never shows.', {
          blockId,
          path: 'content.solution',
          suggestion: 'The reveal only escalates on retry checks — set allow_retry: true or move the content into explanation.',
        }));
      }
      const maxAttempts = block?.content?.max_attempts_before_reveal;
      if (maxAttempts != null && (!Number.isInteger(maxAttempts) || maxAttempts < 1)) {
        add(makeIssue('error', 'check_max_attempts_invalid', 'max_attempts_before_reveal must be a positive integer.', {
          blockId,
          path: 'content.max_attempts_before_reveal',
          suggestion: 'Use an integer ≥ 1, or remove the key to use the default (2).',
        }));
      }
    }
  ...
  list.forEach((block, index) => {
    ...
    // A lesson_complete block is an intentional terminal — it ends the
    // lesson with its own button, so it never warns about a missing next.
    const hasLinearNext = index < list.length - 1;
    const hasAnyPath = Boolean(hasLinearNext || onCorrect || onIncorrect || rejoin);
    if (!hasAnyPath && block.block_type !== 'lesson_complete') {
  ...
  // lesson_complete is a terminal block: at most one, and it must be the
  // last block in the lesson.
  const completeIndexes = list
    .map((b, i) => (b?.block_type === 'lesson_complete' ? i : -1))
    .filter((i) => i >= 0);
  if (completeIndexes.length > 1) {
    add(makeIssue('error', 'multiple_completion_blocks', 'A lesson can have only one Complete-lesson block.', {
      blockId: blockIdOf(list[completeIndexes[completeIndexes.length - 1]], completeIndexes[completeIndexes.length - 1]),
      path: 'block_type',
      suggestion: 'Keep a single lesson_complete block and delete the extras.',
    }));
  }
  for (const i of completeIndexes) {
    if (i !== list.length - 1) {
      add(makeIssue('error', 'completion_block_not_last', 'The Complete-lesson block must be the last block.', {
        blockId: blockIdOf(list[i], i),
        path: 'block_type',
        suggestion: 'Move the lesson_complete block to the end of the lesson.',
      }));
    }
  }
