// Tutor-facing lesson progress reporting: turns lesson_progress rows
// (check_answers entries maintained by applyCheckAttempt in
// runtime-progress.mjs) into the per-check table on the per-student
// assignment report and the struggled-blocks rollup on the assignment
// roster. Pure functions over already-fetched rows — no queries here.

import { checkAttemptCount, checkFirstTryCorrect } from './runtime-progress.mjs';

const ANSWERABLE_BLOCK_TYPES = new Set(['check', 'desmos_interactive']);

// A short plain-text label for a check block. Check prompts and Desmos
// titles are authored as HTML/LaTeX-bearing strings; the report tables
// need a one-line handle, not a rendered block.
export function checkBlockLabel(block, maxLength = 90) {
  const raw =
    block?.block_type === 'desmos_interactive'
      ? block?.content?.title || 'Desmos activity'
      : block?.content?.prompt || 'Knowledge check';
  const text = String(raw)
    .replace(/<[^>]*>/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  if (text.length <= maxLength) return text;
  return `${text.slice(0, maxLength - 1).trimEnd()}…`;
}

// The lesson's answerable blocks, in lesson order, with their 1-indexed
// step numbers (positions among ALL blocks, matching the "Step N of M"
// label the student sees).
export function lessonCheckBlocks(blocks) {
  const rows = [];
  (blocks || []).forEach((block, index) => {
    if (!ANSWERABLE_BLOCK_TYPES.has(block?.block_type)) return;
    rows.push({
      blockId: block.id,
      stepNumber: index + 1,
      blockType: block.block_type,
      label: checkBlockLabel(block),
    });
  });
  return rows;
}

// "Struggled" = the attempt history shows real friction: three or more
// tries, or a final answer that is still wrong. (A one-shot check missed
// on its only try counts — that final wrong answer is the signal.)
export function isStruggledAttempt(entry) {
  if (!entry) return false;
  return checkAttemptCount(entry) >= 3 || !entry.correct;
}

// Resolve a stored lesson_progress row against the lesson's CURRENT
// blocks. Progress is keyed on lesson_blocks.id; a block that was
// deleted (or, before 2026-09-06, simply re-saved — every save minted
// fresh uuids) leaves entries that point at nothing. Those must not be
// counted as steps done or checks answered, and the reader should be
// told they exist rather than shown a number that cannot be right.
export function resolveLessonProgress(blocks, progress) {
  const currentIds = new Set((blocks || []).map((b) => String(b?.id)));
  const storedCompleted = Array.isArray(progress?.completed_blocks) ? progress.completed_blocks : [];
  const completedBlockIds = storedCompleted.filter((id) => currentIds.has(String(id)));
  const checkAnswers = {};
  let staleAnswers = 0;
  for (const [blockId, entry] of Object.entries(progress?.check_answers || {})) {
    if (currentIds.has(String(blockId))) checkAnswers[blockId] = entry;
    else staleAnswers += 1;
  }
  const staleCompleted = storedCompleted.length - completedBlockIds.length;
  return {
    completedBlockIds,
    checkAnswers,
    completedAt: progress?.completed_at ?? null,
    stale: { completed: staleCompleted, answers: staleAnswers },
    hasStale: staleCompleted > 0 || staleAnswers > 0,
  };
}

// Per-student rows for the per-check table: every answerable block in
// lesson order, answered or not, so skipped checks are visible too.
export function buildLessonCheckRows(blocks, checkAnswers) {
  const answers = checkAnswers || {};
  return lessonCheckBlocks(blocks).map((row) => {
    const entry = answers[row.blockId];
    return {
      ...row,
      attempted: Boolean(entry),
      attempts: checkAttemptCount(entry),
      firstTryCorrect: entry ? checkFirstTryCorrect(entry) : null,
      finalCorrect: entry ? Boolean(entry.correct) : null,
      struggled: isStruggledAttempt(entry),
    };
  });
}

// Roster-level rollup: for each answerable block, how the cohort did.
// progressRows is the lesson_progress rows for the enrolled students
// (missing rows mean the student never started — they simply don't
// count toward attemptedCount). Keyed on current block ids, so stale
// entries (see resolveLessonProgress) fall out on their own.
export function buildLessonCheckRollup(blocks, progressRows) {
  const answerSets = (progressRows || []).map((row) => row?.check_answers || {});
  return lessonCheckBlocks(blocks).map((row) => {
    let attemptedCount = 0;
    let firstTryCount = 0;
    let struggledCount = 0;
    let totalAttempts = 0;
    for (const answers of answerSets) {
      const entry = answers[row.blockId];
      if (!entry) continue;
      attemptedCount += 1;
      totalAttempts += checkAttemptCount(entry);
      if (checkFirstTryCorrect(entry)) firstTryCount += 1;
      if (isStruggledAttempt(entry)) struggledCount += 1;
    }
    return {
      ...row,
      attemptedCount,
      firstTryCount,
      struggledCount,
      avgAttempts:
        attemptedCount > 0
          ? Math.round((totalAttempts / attemptedCount) * 10) / 10
          : null,
    };
  });
}

// Lesson-level efficacy rollup for /admin/lessons (plan 5.5): how the
// lesson's checks actually performed across everyone who has taken it.
//
// Unlike the rollups above this takes NO blocks argument. Every
// check_answers entry already carries its own first_try_correct and
// attempts array, so a lesson-level number needs the progress rows
// alone — which keeps the admin list page off a blocks join it would
// otherwise have to chunk around the PostgREST row cap.
//
// Desmos entries are excluded: a gated activity's attempt count
// measures the gate, not the item, and mixing the two would blur
// exactly the distinction this column exists to draw (a hard item vs.
// a broken one). applyCheckAttempt tags only Desmos entries with a
// `type`, so an untagged entry is a knowledge check.
export function summarizeLessonEfficacy(progressRows) {
  let students = 0;
  let answered = 0;
  let tracked = 0;
  let firstTryCorrect = 0;
  let struggled = 0;
  let trackedAttempts = 0;

  for (const row of progressRows || []) {
    const answers = row?.check_answers || {};
    let rowAnswered = 0;
    for (const entry of Object.values(answers)) {
      if (!entry || entry.type === 'desmos_interactive') continue;
      rowAnswered += 1;
      if (isStruggledAttempt(entry)) struggled += 1;
      // Only entries written since Phase 1.1 carry a real first-try
      // signal. checkFirstTryCorrect falls back to final correctness
      // for older ones, which reads every finalized entry as a
      // first-try success — a systematic upward bias that would show
      // legacy-only lessons as a confident 100%. The rate is therefore
      // computed over tracked entries alone.
      if (typeof entry.first_try_correct !== 'boolean') continue;
      tracked += 1;
      trackedAttempts += checkAttemptCount(entry);
      if (entry.first_try_correct) firstTryCorrect += 1;
    }
    if (rowAnswered > 0) students += 1;
    answered += rowAnswered;
  }

  return {
    students,
    answered,
    tracked,
    firstTryCorrect,
    struggled,
    // Null rather than 0 when there is no tracked data — "not measured
    // yet" and "everyone got it wrong" must not render the same.
    firstTryRate: tracked > 0 ? firstTryCorrect / tracked : null,
    avgAttempts:
      tracked > 0 ? Math.round((trackedAttempts / tracked) * 10) / 10 : null,
  };
}

// Per-student first-try tally across the checks they submitted — the
// roster table's accuracy column for lesson assignments.
export function tallyFirstTry(blocks, checkAnswers) {
  const rows = buildLessonCheckRows(blocks, checkAnswers).filter((r) => r.attempted);
  return {
    answered: rows.length,
    firstTryCorrect: rows.filter((r) => r.firstTryCorrect).length,
  };
}
