...
test('a check that refers to an off-slide passage fires', () => {
  const result = lintLessonBlocks([
    text('t1', '<p>Read this:</p><blockquote><p>Museums sometimes display replicas.</p></blockquote>'),
    check('c1', {
      prompt: 'Which answer choice matches the pre-answer and the museum passage?',
      choices: ['Replicas cost less.', 'Context helps visitors learn.'],
      correct_index: 1,
    }),
    check('c2', {
      prompt: 'Which choice completes the sentence?',
      choices: ['reveal', 'reveals'],
      correct_index: 1,
    }),
  ], { title: 'Reading Process' });
  const hits = result.warnings.filter((w) => w.code === 'lint_missing_context');
  assert.deepEqual(hits.map((w) => w.blockId), ['c1', 'c2']);
});

test('a pinned context satisfies a passage reference', () => {
  const result = lintLessonBlocks([
    check('c1', {
      prompt: 'Which answer choice matches the pre-answer and the museum passage?',
      choices: ['Replicas cost less.', 'Context helps visitors learn.'],
      correct_index: 1,
      context: { html: '<p>Museums sometimes display replicas.</p>', label: 'Passage' },
    }),
  ], { title: 'Reading Process' });
  assert.equal(result.warnings.filter((w) => w.code === 'lint_missing_context').length, 0);
});

test('self-contained prompts do not fire the context rule', () => {
  const result = lintLessonBlocks([
    // The sentence follows "this sentence:" inline.
    check('c1', {
      prompt: 'Why are semicolons appropriate in this sentence? The speakers were Ana Ruiz, a chemist; Marcus Lee, an engineer.',
      choices: ['They separate list items that contain commas.', 'They join two questions.'],
      correct_index: 0,
    }),
    // A fill-in blank carries the sentence.
    check('c2', {
      prompt: 'Known for a mirrored dome, _____. Which choice completes the text?',
      choices: ['the library opened in 1990', 'visitors admired the library'],
      correct_index: 0,
    }),
    // A quoted SAT stem is an example, not a reference.
    check('c3', {
      prompt: '"Which choice best states the main idea of the text?" — from memory, what do you do first?',
      choices: ['Read the choices.', 'Form an answer target.'],
      correct_index: 1,
    }),
    // A paraphrase opener supplies the material.
    check('c4', {
      prompt: 'The passage explains that Mateo Ruiz tracked tortoises with radio tags. Which choice completes the text?',
      choices: ['Ruiz mapped the routes.', 'the routes were mapped.'],
      correct_index: 0,
    }),
  ], { title: 'Boundaries' });
  assert.equal(result.warnings.filter((w) => w.code === 'lint_missing_context').length, 0);
...
// ─── retired cadences (plan 6.3) ────────────────────────────────

test('lint_retired_tail flags a "Next, you will" preview in an explanation', () => {
  const tail = check('t', { ...cleanCheck('t').content,
    explanation: 'Right. The intercepts are (2,0) and (3,0). Next, you will name the axis.' });
  assert.ok(codes(lintLessonBlocks([tail])).includes('lint_retired_tail'));
  const contraction = check('t', { ...cleanCheck('t').content,
    explanation: "Yes — and next you'll see why the second coordinate is zero." });
  assert.ok(codes(lintLessonBlocks([contraction])).includes('lint_retired_tail'));
  assert.ok(!codes(lintLessonBlocks([cleanCheck('t')])).includes('lint_retired_tail'));
});

test('lint_retired_closer flags the stock retrieval stem, on either answer surface', () => {
  const stock = check('r', { ...cleanCheck('r').content,
    prompt: 'Without looking back, which sequence correctly solves an equation with x-intercepts?' });
  assert.ok(codes(lintLessonBlocks([stock])).includes('lint_retired_closer'));
  const numericStock = check('r', { prompt: 'Without looking back, which value…', input: 'numeric', answer: '4' });
  assert.ok(codes(lintLessonBlocks([numericStock])).includes('lint_retired_closer'));
  const voiced = check('r', { ...cleanCheck('r').content,
    prompt: 'Cover the screen. A friend asks how you solve an equation by graphing — in order, what do you say?' });
  assert.ok(!codes(lintLessonBlocks([voiced])).includes('lint_retired_closer'));
});

test('affirmation-opener rate is a stat, not a warning', () => {
  const uniform = lintLessonBlocks([
    check('a', { ...cleanCheck('a').content, explanation: 'Correct. Because 2x = 8.' }),
    check('b', { ...cleanCheck('b').content, explanation: 'Exactly — subtract first.' }),
    check('c', { ...cleanCheck('c').content, explanation: 'The coefficient is the slope.' }),
  ]);
  assert.equal(uniform.stats.explanationCount, 3);
  assert.equal(uniform.stats.affirmationOpeners, 2);
  assert.ok(Math.abs(uniform.stats.affirmationOpenerRate - 2 / 3) < 1e-9);
  assert.ok(!codes(uniform).some((c) => /affirmation/.test(c)));
});
