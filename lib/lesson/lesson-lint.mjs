...
import { isNumericCheck, numericCheckAnswers } from './numeric-check.mjs';
...
// A check prompt pointing at text material — "the passage", "the
// museum table", "the previous triangle" — that a learner can only
// have seen on an earlier slide. "this sentence" is excluded on
// purpose: authors write "this sentence: <the sentence>" when the
// material follows inline.
const CONTEXT_REFERENCE =
  /\bthe (?:[\w-]+ )?(?:passage|text|sentence|sentences|excerpt|paragraph|table|study|introduction)\b(?!-)|\bthe previous (?:block|slide|triangle|example|passage|sentence|table)\b/i;
// Signals that the prompt carries the material itself: a fill-in
// blank (the sentence is right there), a paraphrase opener ("The
// passage explains that…"), or a hypothetical ("a passage", "a
// choice") rather than a specific one.
const CONTEXT_SELF_CONTAINED =
  /_{3,}|\b(?:the passage|the text) (?:says|explains|compares|describes|states|notes|reports)\b|\b(?:a|any|each) (?:choice|passage|sentence|table)\b/i;
// A quoted SAT stem ("Which choice best states the main idea of the
// text?") is an example of a question, not a reference to a text.
const QUOTED_SPAN = /"[^"]*"|\u201c[^\u201d]*\u201d|'[^']*\?'/g;
...
export function lintLessonBlocks(blocks, { title = '' } = {}) {
  ...
  let numericChecks = 0;
  // House-voice cadence (plan 6.3): how many explanations open with the
  // same stock affirmation. Reported as a rate, not warned per check —
  // the fix is variety across the lesson, not a change to any one item.
  let explanationCount = 0;
  let affirmationOpeners = 0;

  // (d′) a hint that states the keyed number. Shared by both answer
  // surfaces: "= 12" in a hint gives away a numeric-entry answer just
  // as surely as a keyed choice.
  const hintStatesNumber = (block, index, hint, numbers) => {
    const hintNorm = normalizeItemText(hint).toLowerCase();
  ...
  for (const { block, index } of checks) {
    ...
    // (j) retired cadences (plan 6.3) — these read the prompt and the
    // explanation, so they apply to both answer surfaces.
    const explanation = String(c.explanation ?? '').trim();
    if (explanation) {
      explanationCount += 1;
      if (/^(?:correct|right|exactly|yes)\b/i.test(explanation)) affirmationOpeners += 1;
      if (/\bnext,? you(?: will|'ll)\b/i.test(explanation)) {
        warn('lint_retired_tail', block, index,
          'Explanation carries a "Next, you will…" tail — a check explains the answer; it does not preview the next slide.');
      }
    }
    if (/^\s*without looking back,?\s+which\b/i.test(String(c.prompt ?? ''))) {
      warn('lint_retired_closer', block, index,
        'Stock retrieval stem "Without looking back, which…" — write the retrieval prompt in the lesson\'s own voice.');
    }

    // Numeric-entry checks (plan 1.6) have no choices to lint; only the
    // hint rule applies.
    if (isNumericCheck(c)) {
      numericChecks += 1;
      if (c.hint) {
        const nums = numericCheckAnswers(c).flatMap((a) => a.match(/-?\d+(?:\.\d+)?/g) || []);
        hintStatesNumber(block, index, c.hint, nums);
      }
      continue;
    }
    ...
    if (c.hint) {
      ...
      if (keyed.length >= 4 && hintNorm.includes(keyed.toLowerCase())) {
        ...
        hintStatesNumber(block, index, c.hint, keyed.match(/-?\d+(?:\.\d+)?/g) || []);
      }
    }
    ...
    // (i) SPR candidate: a one-shot measurement check whose every choice
    // is a bare number (not a ratio or a percent — those aren't
    // student-produced-response answers). The SAT asks these as a
    // typed value; as multiple-choice the distractors do little
    // diagnostic work and the format itself is inauthentic (plan 1.6).
    // Teaching checks with retry keep their choices — there the
    // distractors and the hint are the point.
    const numericKinds = norm.map((s) => choiceNumericValue(s)?.kind ?? null);
    if (!c.allow_retry && numericKinds.every((k) => k === 'number')) {
      warn('lint_spr_candidate', block, index,
        'One-shot check with all-numeric choices — the SAT would ask this as a typed answer; consider input: "numeric".');
    }
  ...
  // (g2) check prompts that refer to a passage / sentence / table
  // with nothing pinned on the slide. The slideshow shows one block
  // at a time, so material introduced on an earlier slide is gone by
  // the time the check appears; `content.context` (the text twin of
  // the pinned figure) keeps it on screen.
  list.forEach((block, index) => {
    if (block?.block_type !== 'check') return;
    const c = block?.content || {};
    const prompt = String(c.prompt || '').replace(QUOTED_SPAN, ' ');
    if (!CONTEXT_REFERENCE.test(prompt)) return;
    if (CONTEXT_SELF_CONTAINED.test(prompt)) return;
    // A prompt long enough to carry the passage itself is self-contained.
    if (prompt.length >= 220) return;
    if (typeof c.context?.html === 'string' && c.context.html.trim()) return;
    warn('lint_missing_context', block, index,
      'Prompt refers to a passage, sentence, or table that is not on this slide — pin it with a block-level `context` or restate it in the prompt.');
  });
  ...
  return {
    ...
    stats: {
      ...
      numericCheckCount: numericChecks,
      explanationCount,
      affirmationOpeners,
      affirmationOpenerRate: explanationCount > 0 ? affirmationOpeners / explanationCount : 0,
      keyedLongestCount,
