...
test('resolveLessonProgress drops entries that point at blocks the lesson no longer has', () => {
  const progress = {
    completed_blocks: ['intro', 'c1', 'old-a', 'old-b'],
    check_answers: {
      c1: entryFrom([{ selected: 'x', correct: true }]),
      'old-c': entryFrom([{ selected: 'y', correct: false }]),
    },
    completed_at: null,
  };
  const out = resolveLessonProgress(BLOCKS, progress);
  assert.deepEqual(out.completedBlockIds, ['intro', 'c1']);
  assert.deepEqual(Object.keys(out.checkAnswers), ['c1']);
  assert.deepEqual(out.stale, { completed: 2, answers: 1 });
  assert.equal(out.hasStale, true);
  assert.equal(out.completedAt, null);
});

test('resolveLessonProgress is a no-op on a fresh or fully current row', () => {
  assert.deepEqual(resolveLessonProgress(BLOCKS, null).completedBlockIds, []);
  assert.equal(resolveLessonProgress(BLOCKS, null).hasStale, false);
  const out = resolveLessonProgress(BLOCKS, {
    completed_blocks: ['intro'],
    check_answers: { c2: entryFrom([{ correct: true }]) },
    completed_at: '2026-09-01T00:00:00Z',
  });
  assert.equal(out.hasStale, false);
  assert.equal(out.completedAt, '2026-09-01T00:00:00Z');
});
