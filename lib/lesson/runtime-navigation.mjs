...
// Where a returning learner should land when they reopen a lesson.
//
// Uses "furthest block reached + 1" rather than "first incomplete
// block": a learner who branched past a remediation detour has
// incomplete blocks behind them, and dropping them back into slides the
// branch deliberately skipped is worse than starting one step ahead. A
// finished lesson reopens at the top so it reads as review.
export function resolveResumeIndex({
  ...
  if (!blocks.length || !completedBlockIds?.length) return 0;
  const done = new Set(completedBlockIds.map((id) => String(id)));
  let furthest = -1;
  blocks.forEach((block, index) => {
    if (block?.id != null && done.has(String(block.id))) furthest = index;
  });
  if (furthest < 0) return 0;
  return clampIndex(furthest + 1, blocks.length);
}
