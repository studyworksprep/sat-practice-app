...
test('normalizeExpression agrees between a typed Desmos row and the spec form', () => {
  // Desmos stores a typed exponent braced; specs write it bare.
  assert.equal(normalizeExpression('r\\left(x\\right)=cx^{2}+4'), normalizeExpression('r(x)=cx^2+4'));
  assert.equal(normalizeExpression('y=x^{2}-1'), 'y=x^2-1');
  // Multi-token exponents keep their grouping on both sides.
  assert.equal(normalizeExpression('2^{x+1}'), normalizeExpression('2^(x+1)'));
  assert.notEqual(normalizeExpression('2^{x+1}'), normalizeExpression('2^x+1'));
  // The regression operator: Desmos emits \sim, an author may write ~.
  assert.equal(normalizeExpression('r(c)~31'), normalizeExpression('r\\left(c\\right)\\sim31'));
});

test('Functions block 31: the two rows as Desmos stores them pass the normalized match', () => {
  const content = {
    type: 'desmos_interactive',
    title: 'Define r and solve for c',
    instructions_html: '<p>Enter both rows.</p>',
    initial_expressions: [],
    goal: { type: 'multi_expression', required_count: 2 },
    validation: {
      mode: 'normalized',
      expected: ['r(x)=cx^2+4', 'r(c)\\sim31'],
      state_rules: { min_expressions: 2, max_expressions: 2, require_visible_only: true },
      expected_parameters: [{ name: 'c', value: 3 }],
    },
    feedback: { success_message_html: '<p>ok</p>', retry_message_html: '<p>retry</p>' },
    progression: { require_success: true },
  };
  const studentExpressions = [
    { latex: 'r\\left(x\\right)=cx^{2}+4', hidden: false, type: 'expression', regressionParameters: null },
    { latex: 'r\\left(c\\right)\\sim31', hidden: false, type: 'expression', regressionParameters: { c: 3 } },
  ];
  const result = validateDesmosSubmission({ content, studentExpressions, attempts: 1 });
...
// Straight from a live calculator: Add Item → Table names its columns
// x_{1} and y_{1}, and a regression typed in the UI comes back with
// \\sim and \\left(…\\right). The spec spells the columns x_1 and y_1.
test('a table created in the Desmos UI (x_{1}, y_{1}) satisfies require_table columns x_1, y_1', () => {
  const result = customRegressionSubmission([
    {
      type: 'table',
      latex: '',
      columns: [
        { latex: 'x_{1}', values: ['2', '4', ''] },
        { latex: 'y_{1}', values: ['5', '17', ''] },
      ],
    },
    regressionRow('y_{1}\\sim a\\left(x_{1}-2\\right)^{2}+5', { a: 3 }),
  ]);
  assert.equal(result.success, true, JSON.stringify(result.reasons));
});

test('a fitted constant reported as a_{1} matches expected_parameters name a_1', () => {
  const content = structuredClone(DESMOS_INTERACTIVE_REGRESSION_EXAMPLE);
  content.validation.expected_parameters = [{ name: 'a_1', value: 3 }];
  const result = validateDesmosSubmission({
    content,
    studentExpressions: [
      table([2, 4], [5, 17], { xName: 'x_{1}', yName: 'y_{1}' }),
      regressionRow('y_{1}\\sim a_{1}\\left(x_{1}-2\\right)^{2}+5', { 'a_{1}': 3 }),
    ],
    evaluateAtX: undefined,
    attempts: 1,
  });
