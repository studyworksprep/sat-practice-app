...
import { isGeneratedContentId, reconcileBlockRows, stableBlockKey, textBlockKey } from './block-identity.ts';
...
test('stableBlockKey prefers content.id, falls back to normalised text, singleton terminal', () => {
  ...
  // Compiler-minted positional ids key on text, not on the id.
  assert.equal(stableBlockKey({ block_type: 'text', content: { id: 'text_7', html: 'Same' } }), 'text|text:same');
  assert.equal(isGeneratedContentId('graph_workflow_3_step2'), true);
  assert.equal(isGeneratedContentId('raw_12_2'), true);
  assert.equal(isGeneratedContentId('angle_observation_check'), false);
  assert.equal(isGeneratedContentId('text_intro'), false);
...
test('a text block whose generated id shifted after an insertion is still matched on its html', () => {
  const existing = [
    { id: 'u-t1', sort_order: 0, block_type: 'text', content: { id: 'text_1', html: '<p>Opener</p>' } },
    { id: 'u-t2', sort_order: 1, block_type: 'text', content: { id: 'text_2', html: '<p>Second</p>' } },
  ];
  const incoming = [
    { block_type: 'text', content: { id: 'text_1', html: '<p>Opener</p>' } },
    { block_type: 'text', content: { id: 'text_2', html: '<p>Brand new aside</p>' } }, // inserted: takes text_2
    { block_type: 'text', content: { id: 'text_3', html: '<p>Second</p>' } },          // the old text_2, renumbered
  ];
  const plan = reconcileBlockRows(existing, incoming);
  assert.deepEqual(plan.rows.map((r) => r.id), ['u-t1', null, 'u-t2']);
  assert.deepEqual(plan.deleteIds, []);
  assert.equal(textBlockKey(incoming[2]), 'text|text:<p>second</p>'.replace('<p>second</p>', 'second'));
});

test('an id match anywhere in the list beats a text match earlier in it', () => {
  const existing = [
    { id: 'u-a', sort_order: 0, block_type: 'check', content: { id: 'q_a', prompt: 'Shared prompt' } },
  ];
  const incoming = [
    { block_type: 'check', content: { prompt: 'Shared prompt' } },            // text-only candidate, listed first
    { block_type: 'check', content: { id: 'q_a', prompt: 'Reworded prompt' } }, // the real q_a
  ];
  const plan = reconcileBlockRows(existing, incoming);
  assert.deepEqual(plan.rows.map((r) => r.id), [null, 'u-a']);
});
