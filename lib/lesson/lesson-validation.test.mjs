...
test('context validates shape on any block', () => {
  const invalid = validateLessonBlocks([
    { id: 'x1', block_type: 'text', content: { html: '<p>x</p>', context: { label: 'Passage' } } },
    { id: 'x2', block_type: 'check', content: { prompt: 'p', choices: ['a', 'b'], correct_index: 0, context: '<p>x</p>' } },
    { id: 'done', block_type: 'lesson_complete', content: { html: '<p>done</p>' } },
  ]);
  const contextErrors = invalid.errors.filter((e) => e.code === 'context_invalid');
  assert.equal(contextErrors.length, 2);
  assert.deepEqual(contextErrors.map((e) => e.blockId), ['x1', 'x2']);
  ...
  const valid = validateLessonBlocks([
    {
      ...
      block_type: 'check',
      content: {
        prompt: 'p', choices: ['a', 'b'], correct_index: 0,
        context: { html: '<p>The passage.</p>', label: 'Passage' },
      },
    },
    { id: 'done', block_type: 'lesson_complete', content: { html: '<p>done</p>' } },
  ]);
  assert.equal(valid.errors.filter((e) => e.code === 'context_invalid').length, 0);
});
