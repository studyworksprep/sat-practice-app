...
// Canonical string form for the `normalized` match. latexToInfix keeps
// exponents parenthesised because the arithmetic evaluator needs the
// grouping, but for a string comparison that form must agree with what
// an author writes in a spec: Desmos stores a typed "x^2" as x^{2},
// which latexToInfix renders x^(2), while every spec says x^2 — so a
// correct submission on a normalized-mode block with any exponent was
// rejected (Functions block 31, "Define r and solve for c", 2026-09-06).
// A single-token exponent therefore drops its parentheses; multi-token
// exponents keep them on both sides. The regression operator is
// canonicalised too: Desmos emits \sim, authors may write ~.
...
// Desmos serialises a subscripted name with braces — a table created
// from Add Item → Table has columns x_{1} and y_{1}, and a fitted
// constant a_{1} is reported under that key — while authors write x_1
// and a_1 in specs. Both sides go through this before they are
// compared, so the spec's spelling matches what the calculator hands
// back (Custom Regression block 4 rejected a correct table with
// missing_table, 2026-09-08).
...
function parseStudentTables(studentExpressions) {
  return (studentExpressions || [])
    ...
    .map((row) => ({
      columns: row.columns.map((column) => normalizeIdentifier(column?.latex)),
...
function fittedParameters(rows) {
  ...
  for (const row of rows) {
    ...
    for (const [name, value] of Object.entries(parameters)) {
      if (typeof value === 'number' && Number.isFinite(value)) values.set(normalizeIdentifier(name), value);
    }
  }
  return values;
}
...
export function validateStateRules(studentExpressions, stateRules = {}) {
  ...
  const requireTable = stateRules.require_table;
  if (requireTable) {
    const expectedColumns = (requireTable.columns || []).map(normalizeIdentifier);
...
export function validateExpectedParameters(studentExpressions, expectedParameters = []) {
  ...
  for (const expected of expectedParameters) {
    const name = normalizeIdentifier(expected.name);
