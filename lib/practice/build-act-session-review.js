...
// Matches SLOWEST_TOP_N in build-session-review.js.
  ...
  const [
    ...
    questionIds.length > 0
      ? supabase
          ...
          // Sort by label, not ordinal — defense against the same
          // import bug that fix_act_option_ordinals.sql repaired.
...
function buildTiming(items) {
  ...
  const medianMs = sorted.length
    ...
    : 0;
  // Slowest-first shortlist for the report's "longest questions"
  // panel. Kept in step with build-session-review.js's copy, per
  // the duplication note above.
