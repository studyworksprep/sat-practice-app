...
export async function flagQuestionBroken({ questionId, isBroken }) {
  ...
  const flag = !!isBroken;
  // questions_v2 doesn't carry broken_by / broken_at — those columns
  // live only on the v1 `questions` table. The is_broken bit alone
  // is the audit trail on v2; admin-side reporting can use updated_at.
  const { error } = await supabase
    .from('questions_v2')
    .update({ is_broken: flag, updated_at: new Date().toISOString() })
    .eq('id', questionId);
  if (error) return actionFail(error.message);

  // Track who flagged via the editor's last-fixed audit columns,
  // best-effort — failure here doesn't roll back the flag.
  await supabase
    .from('questions_v2')
    .update({ last_fixed_by: user.id, last_fixed_at: new Date().toISOString() })
    .eq('id', questionId);
...
export async function saveQuestionCorrections({
  ...
  // pass worth recording.
  patch.last_fixed_by = user.id;
  patch.last_fixed_at = new Date().toISOString();
