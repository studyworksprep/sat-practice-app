...
// Domains that get the Desmos saved-state toggle on the review.
// SAT math codes plus ACT 'math'. See the matching set in
// PracticeInteractive for the runner-side gate.
...
export function ReviewInteractive({
  ...
  timing = null,
  assignment = null,
  // §3.3 "Learn it first": lessons tagged to the skills this session
  // missed most. Empty for ACT sessions and while lesson_topics
  // coverage is still sparse; the card simply doesn't render.
  ...
  return (
    <main className={s.container}>
      ...
      <section className={s.metrics}>
        ...
        {/* §3.3 lesson recommendations: for the skills missed most
            this session, point at the tagged published lesson before
            the student grinds more questions on it. */}
        {lessonRecommendations.length > 0 && (
          <div className={s.card}>
            <div className={s.sectionLabel}>Learn it first</div>
            <ul className={s.lessonRecList}>
              {lessonRecommendations.map((rec) => (
                <li key={rec.skillCode} className={s.lessonRecRow}>
                  <span className={s.lessonRecText}>
                    You missed {rec.missed}{' '}
                    {rec.skillName} question{rec.missed === 1 ? '' : 's'}
                    {' — here’s the lesson.'}
                  </span>
                  <Link
                    href={`/learn/${rec.lessonId}`}
                    className={s.lessonRecLink}
                  >
                    {rec.lessonTitle} →
                  </Link>
                </li>
