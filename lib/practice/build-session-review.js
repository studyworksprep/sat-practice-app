...
import { loadTechniqueCatalog, loadQuestionTechniqueIds, canTagTechniques } from '@/lib/practice/load-question-techniques';

const MATH_DOMAIN_CODES = new Set(['H', 'P', 'Q', 'S']);
// Cap on the §3.3 "Learn it first" recommendations in the report.
const LESSON_RECS_TOP_N = 3;
// Cap on timing.slowest — the "longest questions" panel is a
// shortlist a tutor scans, not a full ranking (the timing band
// right above it already shows every question).
...
export async function buildSessionReview({
  ...
  // assignments_v2.question_ids and practice_sessions.question_ids are
  // exclusively v2-keyed (SAT) or act_questions-keyed (ACT, handled by
  // the early-return above); no v1 translation needed.
  const v2QueryIds = Array.from(new Set(questionIds));
  const attemptQuestionIds = v2QueryIds;
  // expandLegacyIds was a hook for pre-cutover synthetic-session
  // callers; the helper is now a passthrough since attempts.question_id
  // is also v2-only after Stage E-4. The flag is retained for the
  // caller-signature shape; callers will get cleaned up separately.
  void expandLegacyIds;
  void expandToAttemptIds;
  ...
  // gave during the session. Later attempts (re-runs) are collected
  // separately so the report can list 2nd/3rd/… below the primary.
  const firstAttemptByQid = new Map();
  const laterAttemptsByQid = new Map();
  for (const a of attempts ?? []) {
    const qKey = a.question_id;
  ...
  // The decorations below (Desmos states, concept tags, tutor
  // question notes, student notes, error notes) and the assignment
  // context are mutually independent — one concurrent batch instead
  // of the serial tail of round-trips this used to be.
  const conceptTagsCanTag = CONCEPT_TAGS_CAN_TAG_ROLES.has(role);
  const conceptTagsCanDelete = CONCEPT_TAGS_CAN_TAG_ROLES.has(role);
  const presentQids = items.filter((it) => !it.missing).map((it) => it.questionId);

  // §3.3: missed-skill tally for the "Learn it first" card — how many
  // questions went wrong per skill_code, so the report can point at
  // the tagged published lesson for the worst offenders.
  const missedBySkill = new Map(); // skill_code → { skillName, missed }
  for (const it of items) {
    if (it.missing || it.status !== 'incorrect') continue;
    const code = it.taxonomy?.skill_code;
    if (!code) continue;
    const entry = missedBySkill.get(code) ?? {
      skillName: it.taxonomy?.skill_name ?? code,
      missed: 0,
    };
    entry.missed += 1;
    missedBySkill.set(code, entry);
  }
  ...
  const [
    ...
    // Concept tags — manager/admin only. question_concept_tags is
    // v2-keyed (FK to questions_v2), so the lookup is a direct IN.
    conceptTagsCanTag
    ...
    conceptTagsCanTag && presentQids.length > 0
      ...
      : Promise.resolve({ data: [] }),
    // Question notes — org-scoped tutor notes attached to each item.
    // Loader returns canView=false for non-tutor callers so student
    // review surfaces stay zero-cost.
    ...
    // popover. Owner-only RLS scopes the read to the calling user;
    // the viewer is the owner on every review surface.
    ...
    // §3.3 lesson recommendations for the missed skills. Best-effort
    // (the helper returns an empty map on error) and usually empty
    // while lesson_topics coverage is still being authored.
    missedBySkill.size > 0
      ? recommendLessonsForSkills(supabase, Array.from(missedBySkill.keys()))
  ...
  // Technique catalog + each question's tags, for the picker in the
  // review surfaces (foundations-and-question-patterns §8). Same
  // manager/admin bar as the concept tags, enforced again inside
  // set_question_techniques().
  const techniquesCanTag = canTagTechniques(role);
  const techniqueCatalog = await loadTechniqueCatalog({ role });
  if (techniquesCanTag) {
    const techniquesByQid = await loadQuestionTechniqueIds({
      questionIds: items.filter((it) => !it.missing).map((it) => it.questionId),
    });
    for (const it of items) {
      if (!it.missing) it.techniqueIds = techniquesByQid.get(it.questionId) ?? [];
    }
  }
  ...
  // Worst-missed skills first; capped so the card stays a nudge,
  // not a syllabus. Skills without a tagged published lesson drop
  // out silently.
  const lessonRecommendations = Array.from(missedBySkill.entries())
    .map(([skillCode, m]) => {
      const rec = skillLessonRecs.get(skillCode)?.[0];
      if (!rec) return null;
      return {
        skillCode,
    ...
    .filter(Boolean)
    .sort((a, b) => b.missed - a.missed || a.skillName.localeCompare(b.skillName))
    .slice(0, LESSON_RECS_TOP_N);
...
// by the timing band and its tooltip, and a slowest-first list
// for the report's "longest questions" panel.
// ──────────────────────────────────────────────────────────────
...
function buildTiming(items) {
  ...
  const medianMs = sorted.length
    ...
    : 0;
  // Slowest questions first, so a report can point straight at the
  // time sinks. Same idea as the practice-test report's "slowest 5"
  // tiles, but derived here — that copy lives inside
  // lib/practice-test/load-test-results.js and is keyed to
  // module/ordinal, so no session surface could reuse it. Ties
  // break on position to keep the order stable across renders.
  const slowest = measured
    .slice()
    .sort((a, b) => b.timeSpentMs - a.timeSpentMs || a.position - b.position)
