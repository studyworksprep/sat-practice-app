...
export async function buildSessionReview({
  ...
  // Normalize each attempt's question_id back to whichever id
  // (v1 or v2) the assignment's questionIds array uses, so
  // attempts and items line up regardless of when the question
  // was answered (pre- or post-cutover).
...
async function buildAssignmentContext({ supabase, target, session, questionIds, attemptQuestionIds = null, assignmentRow = null }) {
  const assignmentId =
    session.filter_criteria
    && typeof session.filter_criteria === 'object'
