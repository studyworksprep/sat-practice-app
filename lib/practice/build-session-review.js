...
export async function buildSessionReview({
  ...
  const items = questionIds.map((qid, position) => {
    ...
    return {
      ...
      // Current sub-skill classification, for the pattern picker in
      // the review surfaces (foundations-and-question-patterns §3.4).
  ...
  // Pattern catalog for the skills actually on this report, so the
  // picker offers the 2-6 patterns that could apply to each question
  // rather than the whole vocabulary. Same manager/admin bar as the
  // concept tags, enforced again inside set_question_pattern().
  const questionPatternsCanTag = canTagPatterns(role);
  ...
  return {
    ...
    questionPatternsCatalog,
    questionPatternsCanTag,
