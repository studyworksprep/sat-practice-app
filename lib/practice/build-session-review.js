...
export async function buildSessionReview({
  ...
  // the page off.
  let attemptQuestionIds = questionIds;
  let v2ByLegacy = new Map();
  if (expandLegacyIds && questionIds.length > 0) {
    const expanded = await expandToAttemptIds(supabase, questionIds);
  ...
  // Normalize each attempt's question_id back to its v2 id so
  // legacy + v2 attempts on the same question collapse correctly.
...
async function buildAssignmentContext({ supabase, target, session, questionIds, assignmentRow = null }) {
  const assignmentId =
    session.filter_criteria
    && typeof session.filter_criteria === 'object'
