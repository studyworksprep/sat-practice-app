...
export function StartInteractive({
  ...
  const skillOptions = useMemo(() => {
    ...
    for (const d of domains) {
      ...
      for (const s of d.skills) set.set(s, s);
    }
    ...
    <main style={S.main}>
      ...
      <form onSubmit={onSubmit} style={S.form}>
        ...
        <fieldset style={S.fieldset}>
          ...
          <label style={{ ...S.checkItem, marginTop: '0.5rem' }}>
            <span>Order</span>
        ...
        <fieldset style={S.fieldset}>
          ...
          <input
            ...
            style={S.sizeInput}
          />
          <div style={S.countLine} aria-live="polite">
            {isCounting && count == null && 'Loading count…'}
            {countErr && <span style={{ color: '#b91c1c' }}>{countErr}</span>}
            {count != null && !countErr && (
              <>
                <strong>{count.toLocaleString()}</strong> question{count === 1 ? '' : 's'} match
                {isCounting ? ' (updating…)' : ''}.{' '}
                {count < size
                  ? <span style={{ color: '#92400e' }}>Session will contain {actualSize}.</span>
                  : <span style={{ color: '#6b7280' }}>Session will contain {actualSize}.</span>}
              </>
            )}
          </div>
        </fieldset>

        <Button type="submit" disabled={isSubmitting || count === 0} style={{ alignSelf: 'flex-start' }}>
          {isSubmitting ? 'Starting…' : 'Start session'}
        </Button>

        {submitState && !submitState.ok && (
          <p role="alert" style={S.error}>{submitState.error}</p>
        )}
      </form>
    </main>
  );
  ...
  return (
    <fieldset style={S.fieldset}>
      ...
      <div style={S.checkList}>
        {options.length === 0 && emptyNote && <p style={S.emptyNote}>{emptyNote}</p>}
        {options.map((o) => (
          <label key={String(o.value)} style={S.checkItem}>
            <input
              type="checkbox"
              name={name}
              value={o.value}
...
const S = {
  ...
  resumeLink: { color: '#2563eb', textDecoration: 'none', fontWeight: 600 },
  ...
  checkItem: { display: 'flex', alignItems: 'center', gap: '0.375rem', cursor: 'pointer', fontSize: '0.95rem' },
  emptyNote: { color: '#9ca3af', fontStyle: 'italic' },
  select: { padding: '0.375rem 0.5rem', border: '1px solid #d1d5db', borderRadius: 6, fontSize: '0.95rem' },
  sizeInput: {
    width: 80, padding: '0.4rem 0.6rem',
    border: '1px solid #d1d5db', borderRadius: 6, fontSize: '1rem',
  },
  countLine: { marginTop: '0.5rem', fontSize: '0.9rem', color: '#374151' },
