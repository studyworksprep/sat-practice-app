...
export function StartInteractive({
  ...
  return (
    <main className={s.container}>
      ...
      {tests.length > 0 && (
        <TestLauncher tests={tests} basePath={basePath} />
      )}
...
function TestLauncher({ tests, basePath }) {
  ...
  // "Launch test" goes to the per-test launch page — the instruction
  // screen that feels like a real SAT starting. We pass the selected
  // accommodation via the URL so the launch page initializes with
  // the student's already-picked multiplier and the module times on
  // screen reflect the adjusted clock.
  function launchHref() {
