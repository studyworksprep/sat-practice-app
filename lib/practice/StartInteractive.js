...
export function StartInteractive({
  ...
  searchQuestionsAction = null,
  listTagsAction = null,
  basePath = '/practice',
  historyHref = '/practice/history',
}) {
  // ── Form state ─────────────────────────────────────────────
  ...
  // Opt-in import batches ("Extra practice sets"). Off by default —
  // their questions are outside the standard pool and only join the
  // candidate set while a chip here is selected.
  const [selectedBatches,       setSelectedBatches]       = useState(() => new Set());
  const [unansweredOnly,        setUnansweredOnly]        = useState(false);
  const [order,                 setOrder]                 = useState('display_code');
  // Session size is held as raw input text so the student can edit
  // it freely — clear it, replace a digit, type out of range — and
  // see exactly what they typed. The numeric `size` used by the
  // server actions is derived (parse + clamp), and the input
  // re-snaps to the resolved value on blur.
  ...
  useEffect(() => {
    ...
    selectedBatches, unansweredOnly, countAvailableAction,
  ]);
    ...
    <main className={s.container}>
      ...
      <form onSubmit={onSubmit} className={s.card}>
        ...
        {extraBatches.length > 0 && (
          <>
            <div className={s.chipRow}>
              <span className={s.chipRowLabel}>Extra practice sets</span>
              {extraBatches.map((b) => {
                ...
                return (
                  <button
                    key={b.id}
                    type="button"
                    className={[s.chip, on ? s.on : null].filter(Boolean).join(' ')}
                    aria-pressed={on}
                    onClick={() => toggleSetter(setSelectedBatches)(b.id)}
                  >
                    <span>{b.label}</span>
            ...
            <p className={s.extraSetsHint}>
              Bonus question sets modeled on recent SAT administrations.
              A set joins your session pool only while it&apos;s
              selected — your other filters still apply to it.
            </p>
          </>
        )}
        ...
        <div className={s.actionsRow}>
          <label className={s.secondaryField}>
            ...
            <input
              ...
              onBlur={() => {
                // Snap the visible text to the resolved size when the
                // student finishes editing — empty / out-of-range /
                // non-numeric inputs become the actual session size.
                const next = String(size);
                if (next !== sizeText) setSizeText(next);
              }}
            />
          </label>
          <label className={s.secondaryField}>
      ...
      {searchQuestionsAction && (
        <QuestionSearch
          searchQuestionsAction={searchQuestionsAction}
...
function buildFormData({ skills, difficulties, scoreBands, batches = new Set(), unansweredOnly, order, size }) {
  ...
  for (const v of batches)      fd.append('extra_batch', v);
...
// Parse + clamp the session-size input to a usable number. Empty
// strings and other non-numeric inputs collapse to DEFAULT_SIZE so
// the rest of the page (live count, Start button label, server
// action submission) always has a valid integer to work with even
// while the student is mid-edit.
