...
// Role flags (canTag and canDelete are both manager/admin; delete
// was widened from admin-only 2026-07) drive the inline UI without
// re-checking on the client.
//
...
export async function loadConceptTags({ questionId, role }) {
  ...
  const canDelete = CAN_TAG_ROLES.has(role);
  ...
  // to manager+admin. Teacher-visible read-only is a follow-up if
