...
export async function loadQuestionNotes({ questionId, role, userId }) {
  ...
  // see directly.
  // Per-question fetch. test_type is implied by the question itself;
  // PR 4 will plumb it from the loader. SAT-only matches every caller today.
...
export async function loadQuestionNotesByQuestion({ questionIds, role, userId }) {
  ...
  // Same per-question scope as loadQuestionNotes; SAT-only for now.
  const svc = createServiceClient();
  const { data: rawNotes } = await svc
    .from('question_notes')
    .select(
