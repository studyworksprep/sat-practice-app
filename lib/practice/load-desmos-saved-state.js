 ...
 *   belongs to. Defaults to 'sat'; the ACT runner passes 'act'.
 * @returns {Promise<{ savedState: object|null, canSave: boolean }>}
 */
export async function loadDesmosSavedState({ questionId, role, testType = 'sat' }) {
  if (!questionId) return { savedState: null, canSave: false };

  const supabase = await createClient();

  // SAT saved states are keyed by questions_v2 id; ACT by
  // act_questions id. The same desmos_saved_states table holds both,
  // scoped by the test_type discriminator (PR 1).
  ...
  // SAT-only v1→v2 fallback: a handful of legacy callers may still
  // pass a v1 uuid. ACT has no legacy era, so skip the map lookup.
