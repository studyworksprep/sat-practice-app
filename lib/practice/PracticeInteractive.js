...
// LocalStorage prefix for the per-question Desmos scratch state the
// DesmosPanel persists while a session is open. Keys are scoped by
// SESSION as well as question — `desmos:practice:<sessionId>:<qid>` —
// so graph work follows the student between questions inside one
// session but never leaks into a later session. The key used to be
// per question only, which meant a question re-encountered in a new
// assignment or solo session came up with the student's old graph
// already drawn (reported 2026-09-05).
const DESMOS_SCRATCH_PREFIX = 'desmos:practice:';

// Drop every scratch key that doesn't belong to the current session:
// earlier sessions' keys (so localStorage doesn't grow without
// bound — a Desmos state is a few KB and a student can run hundreds
// of sessions) and the legacy per-question keys, which are exactly
// the stale states that leaked across sessions. Best-effort: any
// storage failure is swallowed, the calculator works without it.
function pruneDesmosScratch(sessionId) {
  try {
    if (typeof window === 'undefined' || !window.localStorage) return;
    const keep = `${DESMOS_SCRATCH_PREFIX}${sessionId}:`;
    const stale = [];
    for (let i = 0; i < window.localStorage.length; i += 1) {
      const key = window.localStorage.key(i);
      if (key && key.startsWith(DESMOS_SCRATCH_PREFIX) && !key.startsWith(keep)) {
        stale.push(key);
      }
    }
...
export function PracticeInteractive({
  ...
  sessionMode = null,
  // The student's own §3.2 switch (profiles.practice_detours_enabled,
  // resolved server-side by lib/practice/detour-preference.mjs).
  // Defaults true so the tutor-training mount, which has no student
  // to read it from, keeps its existing behavior — mode gating
  // already excludes training from detours anyway.
  detoursEnabled = true,
  submitAnswerAction,
  loadQuestionAction,
  ...
  //
  // detoursEnabled is the student's own switch. Off means the miss
  // streak is never even counted, so nothing probes and no card can
  // appear; loadDetourSession enforces the same thing server-side.
  const detourEligible = detoursEnabled
    && (sessionMode === 'practice' || sessionMode === 'review');
  ...
  // activeSinceRef is the baseline of the current ACTIVE segment on
  // the current question; every flush sends the delta since then and
  // advances the baseline optimistically (before the round-trip), so
  // a concurrent flush can't bill the same seconds twice — the same
  // anti-double-count discipline as the test runner. Hidden-tab time
  // is never billed: the hide handler flushes the active segment,
  // and the show handler re-baselines so the hidden gap vanishes.
  //
  // Where a delta lands depends on whether the question has an
  // attempt row yet (first-attempt-wins, created on submit):
  //   * no attempt → pendingMsRef (client memory, keyed by question
  //     id, survives navigation within the session) — it rides on
  //     the submitAnswer call if the student answers later. We never
  //     create placeholder attempts rows for viewing time: an
  //     attempts row means "answered" to mastery, the review queue,
  //     and the session report.
  //   * attempt exists → sendBeacon to /api/practice/time-ping,
  //     which accumulates atomically server-side. Beacon (not a
  //     Server Action) so the pagehide flush survives tab close.
  const activeSinceRef = useRef(Date.now());
  const pendingMsRef = useRef(new Map());
  const questionIdRef = useRef(initialQuestion.questionId);
  const attemptExistsRef = useRef(initialAttempt != null);

  const takeElapsedMs = useCallback(() => {
    const now = Date.now();
    const delta = Math.max(0, now - activeSinceRef.current);
    activeSinceRef.current = now;
    return delta;
  }, []);

  // Flush the current question's active segment. Callable from
  // event handlers, applyPayload (before the refs swap to the new
  // question), and the unmount cleanup — everything it reads lives
  // in refs so it never closes over stale state.
  const flushQuestionTime = useCallback(() => {
    const delta = takeElapsedMs();
    if (delta <= 0) return;
    const qid = questionIdRef.current;
    if (!attemptExistsRef.current) {
      pendingMsRef.current.set(qid, (pendingMsRef.current.get(qid) ?? 0) + delta);
      return;
    }
    // Sub-500ms slivers aren't worth a network call (same threshold
    // as the test runner's beacon).
    if (delta < 500) return;
    try {
      ...
      navigator.sendBeacon('/api/practice/time-ping', blob);
    } catch { /* sendBeacon unsupported — drop the sliver */ }
  }, [sessionId, takeElapsedMs]);

  // visibilitychange/pagehide flush + hidden-time exclusion. pagehide
  // covers tab close / real navigations; SPA navigations (Save &
  // exit, detour lesson links, finish) don't fire it, so the unmount
  // cleanup below is the flush of last resort for those.
  useEffect(() => {
    ...
    function onVisibility() {
      if (document.visibilityState === 'hidden') {
        ...
        // Back from a hidden tab — the gap isn't practice time.
        activeSinceRef.current = Date.now();
      }
    }
    document.addEventListener('visibilitychange', onVisibility);
    window.addEventListener('pagehide', flushQuestionTime);
    return () => {
      document.removeEventListener('visibilitychange', onVisibility);
      window.removeEventListener('pagehide', flushQuestionTime);
    };
  }, [flushQuestionTime]);

  // Unmount flush — the runner suppresses app chrome, so every way
  // out of a live session (Save & exit, lesson link, finish route)
  // is a client-side navigation that unmounts this island without
  // firing pagehide.
  useEffect(() => () => { flushQuestionTime(); }, [flushQuestionTime]);
  ...
  const applyPayload = useCallback((payload) => {
    ...
    const swap = () => {
      // Bill the outgoing question's active segment before the
      // stopwatch refs move to the incoming one. positionRef still
      // points at the old position here (it re-syncs on render), so
      // the beacon lands on the question being left.
      flushQuestionTime();
      questionIdRef.current = payload.question.questionId;
      attemptExistsRef.current = payload.initialAttempt != null;
      activeSinceRef.current = Date.now();
    ...
    if (supportsVT && !reduceMotion) {
      ...
      // doesn't surface as an unhandled-rejection error. (Same
      // treatment as the test runner's applyPayload.)
      vt.finished.catch(() => {});
    } else {
      swap();
    }
  ...
  }, [basePath, sessionId, flushQuestionTime]);
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        // Time rides with the answer too: the current active segment
        // plus any earlier visits to this question that had nowhere
        // to land yet (no attempt row until now). Baseline advances
        // optimistically — a failed submit drops the delta rather
        // than risking a double-bill (same tradeoff as the test
        // runner). Re-submits accumulate server-side.
        const priorMs = pendingMsRef.current.get(question.questionId) ?? 0;
        pendingMsRef.current.delete(question.questionId);
        const timeSpentMs = takeElapsedMs() + priorMs;
        if (timeSpentMs > 0) fd.set('timeSpentMs', String(timeSpentMs));
        const res = await submitAnswerAction(null, fd);
        if (res?.ok) {
          // An attempt row now exists — later flushes for this
          // question go to the time-ping route, not the pending map.
          attemptExistsRef.current = true;
    ...
    [
      ...
      question.questionId, takeElapsedMs,
    ],
  );
  ...
  // §6.3b: useOptimistic replaces the hand-rolled flip-then-rollback
  // — the optimistic flag applies instantly inside the transition
  // and auto-reverts if the action fails (setMarked never fires).
  // The map cell still patches on confirmation only; the button is
  ...
  const handleToggleMark = useCallback(() => {
    ...
    reactStartTransition(async () => {
      addOptimisticMarked(nextMarked);
      ...
      try {
        ...
        // Optimistic value reverts on settle; nothing to roll back.
      } finally {
        setMarkPending(false);
      }
    });
  }, [marked, markPending, position, sessionId, invalidatePrefetchCache, addOptimisticMarked]);
  ...
  useEffect(() => { pruneDesmosScratch(sessionId); }, [sessionId]);
        ...
        <header className={s.header}>
          ...
          <div className={s.headerRight}>
            ...
            <button
              ...
              className={`${s.markBtn} ${optimisticMarked ? s.markBtnActive : ''}`}
              title={optimisticMarked ? 'Unmark this question' : 'Mark this question for review'}
            >
              <BookmarkIcon filled={marked} size={14} />
              {optimisticMarked ? 'Marked' : 'Mark for review'}
            </button>
            ...
            leftSlot={
              desmosEligible ? (
                <DesmosPanel
                  ...
                  storageKey={`${DESMOS_SCRATCH_PREFIX}${sessionId}:${question.questionId}`}
          ...
          nextPayload && nextPayload.question.questionId !== rendererKey ? (
            <Activity key={nextPayload.question.questionId} mode="hidden">
              <QuestionRenderer
                ...
                subject={nextPayload.question.taxonomy?.domain_code
                  ? domainSection(nextPayload.question.taxonomy.domain_code)
                  : null}
