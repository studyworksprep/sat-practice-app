...
export function PracticeInteractive({
  ...
  //
  // When the student is revisiting a question they already
  // submitted, `initialAttempt` carries the review reveal data
  // (correctOptionId, correctAnswerDisplay, rationaleHtml) loaded
  // server-side via lib/practice/load-review-data.js. Those fields
  // may be null on a fresh question that hasn't been answered yet.
  ...
  const handleNext = useCallback(() => {
    startTransition(() => {
      // Client-side route push — cheap, uses the app's prefetch.
      // basePath and sessionCompleteHref are passed in by the page
      // so the same client island works for both /practice/* and
      // /tutor/training/* without branching on role.
      ...
      <article style={S.article}>
        ...
        {isReviewed && result && (
          <section style={S.result}>
            ...
            <Button type="button" onClick={handleNext} style={{ alignSelf: 'flex-start' }}>
              Next question →
            </Button>
          </section>
        )}
