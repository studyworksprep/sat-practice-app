...
export function PracticeInteractive({
  ...
  // Wider container for two-column reading questions so each pane
  // gets enough room for natural line-breaks instead of wrapping
  // one word per line.
  const mainStyle = {
    ...S.main,
    maxWidth: question.layout === 'two-column' ? 1280 : S.main.maxWidth,
  };
  // When Desmos is open, shift content right and let the remaining
  // viewport fill to the right — override both left and right
  // margins so the layout doesn't try to re-center inside a half-
  // viewport-width band.
  if (desmosVisible) {
    mainStyle.marginLeft = 'min(52vw, 720px)';
    mainStyle.marginRight = space[5];
  }
...
const S = {
  main: {
    maxWidth: 860,
    // Explicit longhand — the `margin: X auto` shorthand was
    // interacting poorly with the conditional marginLeft we set
    // when Desmos is open, leaving content pinned to the left
    // instead of centered.
    marginTop: space[5],
