...
export function PracticeInteractive({
  ...
  const handleNext = useCallback(() => {
    ...
    // Last question — close the session and route out. The
    // submit action is idempotent so a reload during the
    // transition is safe.
    startTransition(async () => {
      const fd = new FormData();
      fd.set('sessionId', sessionId);
  ...
  }, [position, total, navigateTo, sessionId, sessionCompleteHref, router]);
  ...
  const refBtnClass = `${s.navBtn} ${refOpen ? s.navBtnActive : ''}`;
