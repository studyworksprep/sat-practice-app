...
export function PracticeInteractive({
  ...
  // layout — either a reading passage (layout='two-column') or
  // a math question with the Desmos pane available (desmosEligible).
  // Single-column math questions keep the narrower 860px default.
  const needsWideContainer =
    question.layout === 'two-column' || desmosEligible;
  const mainStyle = {
    ...S.main,
    maxWidth: needsWideContainer ? 1280 : S.main.maxWidth,
  };
