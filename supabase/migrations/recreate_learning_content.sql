-- =========================================================
-- Recreate Learning Content System
--
-- The original migration (create_learning_content.sql) had a
-- syntax error in the lesson_topics primary key clause:
--
--   primary key (lesson_id, domain_name, coalesce(skill_code, ''))
--
-- PostgreSQL does not allow function expressions in a
-- PRIMARY KEY declaration — only plain column references are
-- permitted. Unique indexes, however, DO support expressions,
-- so this migration uses a synthetic uuid primary key on
-- lesson_topics and enforces the original uniqueness intent
-- ("one row per lesson/domain/skill, with null skill_code
-- meaning a domain-level tag") via a unique expression index.
--
-- This migration re-creates the full learning content schema
-- (lessons, blocks, topics, assignments, progress) along with
-- all RLS policies and helper functions. It is idempotent and
-- safe to re-run: table creates use IF NOT EXISTS, helper
-- functions use CREATE OR REPLACE, and policies are dropped
...
--
-- Uses a synthetic uuid primary key plus a unique expression
-- index (which Postgres allows) to enforce the intent of the
-- original composite key: one row per (lesson, domain, skill),
-- with a null skill_code meaning "domain-level tag only".
...
create index if not exists lesson_topics_lesson_idx on public.lesson_topics(lesson_id);
...
-- Check if a student is assigned to a specific lesson assignment.
...
-- Check if a user is the teacher who owns a lesson assignment.
...
-- Check if a student has been assigned a specific lesson (by any teacher).
...
-- Can the current user view this lesson? (used by blocks & topics policies)
...
-- Is the current user the author of this lesson?
...
drop policy if exists lesson_topics_select on public.lesson_topics;
...
drop policy if exists lesson_assignments_select on public.lesson_assignments;
...
drop policy if exists lesson_assignments_update on public.lesson_assignments;
...
drop policy if exists las_select on public.lesson_assignment_students;
...
drop policy if exists las_insert on public.lesson_assignment_students;
...
drop policy if exists las_delete on public.lesson_assignment_students;
