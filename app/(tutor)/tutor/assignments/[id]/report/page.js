...
export default async function TutorAssignmentGroupReportPage({ params }) {
  ...
  // Technique catalog + each question's tags (same treatment as
  // build-session-review gives the per-student report).
  const techniquesCanTag = canTagTechniques(profile.role);
  const techniqueCatalog = await loadTechniqueCatalog({ role: profile.role });
  const techniquesByQid = techniquesCanTag
    ? await loadQuestionTechniqueIds({ questionIds: presentQids })
    : new Map();
  ...
  for (const it of items) {
    ...
    if (techniquesCanTag) it.techniqueIds = techniquesByQid.get(it.questionId) ?? [];
  ...
  return (
    <GroupAssignmentReport
      ...
      techniquesCanTag={techniquesCanTag}
      assignment={{
        id: assignment.id,
        title: assignment.title ?? 'Assignment',
