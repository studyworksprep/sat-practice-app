...
export function ImportComparison({showLocalPilot=false,initialSets}:{showLocalPilot?:boolean;initialSets:ImportSetsResult}) {
  ...
  const [batchDestination,setBatchDestination]=useState<ImportChoice['destination']>('regular'), [batchPublish,setBatchPublish]=useState(true), [batchSection,setBatchSection]=useState<ImportChoice['section']>('');
    ...
    try {
      for(let index=0;index<requested.length;index++) {
        ...
        try {
          if(c.preference==='Keep existing') {result={status:'kept',message:'Kept the existing question. No bank changes.',recordId:c.matchId??undefined};kept++;}
          else if(q.matches.length&&!c.importAsNew) {
            ...
            const response=await insertImportedQuestion(q.insertToken!,c.destination==='regular'?null:c.batchId,c.publish,true,c.section,c.importAsNew);
            if(!response.ok)throw new Error(response.error);
  ...
  return <>
    ...
    {batch&&<section aria-label="Question comparison">
      ...
      <section className={s.decisions} aria-label="Batch review settings">
        <h3>Review and approve a batch</h3>
        <p>Select the questions you reviewed, choose shared settings, then approve them together. Questions marked “Needs editing” and unresolved duplicates are skipped. Existing-question replacements keep their individual review decisions.</p>
        <div className={s.destinationGrid}>
          <label className={s.field}>Batch destination<select disabled={locked} value={batchDestination} onChange={e=>{setBatchDestination(e.target.value as ImportChoice['destination']);setBulkConfirm(false);}}><option value="regular">Regular question bank</option><option value="supplemental">Supplemental practice set</option></select></label>
          {batchDestination==='supplemental'&&<label className={s.field}>Batch supplemental set<select disabled={locked} value={defaultBatch} onChange={e=>{setDefaultBatch(e.target.value);setBulkConfirm(false);}}><option value="">Choose a set</option>{sets.ok&&sets.data.sets.map(b=><option key={b.id} value={b.id}>{b.label}</option>)}</select></label>}
          <label className={s.field}>Batch availability<select disabled={locked} value={batchPublish?'published':'draft'} onChange={e=>{setBatchPublish(e.target.value==='published');setBulkConfirm(false);}}><option value="published">Publish for practice</option><option value="draft">Save as unpublished drafts</option></select></label>
          <label className={s.field}>Section when metadata is missing<select disabled={locked} value={batchSection} onChange={e=>{setBatchSection(e.target.value as ImportChoice['section']);setBulkConfirm(false);}}><option value="">Use metadata</option><option value="M">Math</option><option value="RW">Reading &amp; Writing</option></select></label>
        </div>
        <p className={s.small}>By approving, you confirm you reviewed the selected questions’ text, choices and figures, resolved duplicate concerns, and verified answers for publication. Invalid or incomplete questions will be skipped.</p>
        <button className={s.primary} disabled={locked||!selected.length||batch.isSnapshot} onClick={()=>{
          const result=approveBatch(items,Object.fromEntries(items.map(q=>[q.id,choiceFor(q)])),outcomes,{destination:batchDestination,batchId:defaultBatch,publish:batchPublish,section:batchSection});
          setChoices(result.choices);setNotice(`${result.approved.length} approved for import. ${result.skipped.length} skipped; review editing holds, duplicates, or missing metadata separately.`);setBulkConfirm(result.approved.length>0);
        }}>Approve selected for import ({selected.length})</button>
      </section>
      <div className={s.workspace}>
        <aside className={s.sidebar}>
          <div className={s.sidebarTools}><label className={s.check}><input type="checkbox" aria-label="Select all visible questions" disabled={locked||!visible.some(q=>!completed(outcomes[q.id]))} checked={visible.some(q=>!completed(outcomes[q.id]))&&visible.filter(q=>!completed(outcomes[q.id])).every(q=>choiceFor(q).selected)} onChange={e=>{const checked=e.target.checked;setChoices(current=>({...current,...Object.fromEntries(visible.filter(q=>!completed(outcomes[q.id])).map(q=>[q.id,{...choiceFor(q),selected:checked}]))}));setBulkConfirm(false);}}/> Select all {filter!=='all'||search?'visible ':''}questions</label><label className={s.field}>Find a question<input type="search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Question ID or bank code" disabled={locked}/></label><label className={s.field}>Show<select disabled={locked} value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">All questions</option><option value="pending">Not reviewed</option><option value="selected">Selected for import</option><option value="attention">Needs attention</option><option value="errors">Import failed</option><option value="completed">Completed</option></select></label><div className={s.actions}><button disabled={locked||!ready.length} onClick={()=>{setChoices(current=>({...current,...Object.fromEntries(ready.map(q=>[q.id,{...choiceFor(q),selected:true}]))}));setBulkConfirm(false);}}>Select reviewed</button><button disabled={locked||!selected.length} onClick={()=>{setChoices(current=>Object.fromEntries(Object.entries(current).map(([id,c])=>[id,{...c,selected:false}])));setBulkConfirm(false);}}>Clear</button></div></div>
        ...
        {item?<div className={s.detail}>
          ...
          <section className={s.decisions} aria-label="Question review">
            ...
            <div className={s.preferenceGroup}>{(['Keep existing','Prefer imported','Needs editing'] as const).map(value=><button key={value} aria-pressed={choice.preference===value} disabled={locked||completed(outcome)||(!match&&value==='Keep existing')||(item.matches.length>0&&!match&&!choice.importAsNew&&value==='Prefer imported')} onClick={()=>patch(item,{preference:value,...(value==='Keep existing'?{importAsNew:false}:{})})}>{value}</button>)}</div>
            {!!item.matches.length&&<div><label className={s.reviewCheck}><input type="checkbox" disabled={locked||completed(outcome)||!item.insertToken} checked={choice.importAsNew} onChange={e=>patch(item,{importAsNew:e.target.checked,preference:e.target.checked?'Prefer imported':'',stimulusIncluded:false})}/><span><strong>Not a duplicate — import as a new question</strong><small>I compared all suggested matches and verified that this is a different question. Existing bank questions will be left unchanged.</small></span></label>{item.insertBlocked&&<p className={s.small}>{item.insertBlocked}</p>}</div>}
            {!choice.importAsNew&&match?.requiresStimulusConfirmation&&choice.preference==='Prefer imported'&&<label className={s.reviewCheck}><input type="checkbox" disabled={locked||completed(outcome)} checked={choice.stimulusIncluded} onChange={e=>patch(item,{stimulusIncluded:e.target.checked})}/><span><strong>Imported prompt includes the stimulus</strong><small>I verified that the imported preview contains all equations, figures, and text from the existing stimulus. Importing will combine them in the prompt and clear the old separate stimulus, so it will not appear twice. The answer and metadata stay unchanged.</small></span></label>}
            {(!item.matches.length||choice.importAsNew)&&choice.preference==='Prefer imported'&&<div className={s.destinationGrid}>
