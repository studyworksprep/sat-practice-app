...
export function ImportComparison({ showLocalPilot = false }: { showLocalPilot?: boolean }) {
  ...
  async function load(event: FormEvent<HTMLFormElement> | null, pilot = false) {
    ...
    try {
      ...
      setMatches({}); setDecisions({}); setApplied({}); setConfirmed(false); setFilter('all');
  ...
  return <>
    <section className={s.upload} aria-label="Import files">
      <div className={s.sectionHead}><div><h2>1. Choose your files</h2><p>Up to 100 questions. Use the Mathpix export with Question ID headings. Metadata is optional.</p></div>{showLocalPilot && <button type="button" disabled={busy || applying} onClick={() => load(null,true)}>Load local math pilot</button>}</div>
      <form onSubmit={load}>
    ...
    {batch && <section aria-label="Question comparison">
      <div className={s.sectionHead}><div><h2>2. Review the import</h2><p>{batch.items.length} questions · {batch.items.filter(q => q.matches.length).length} with identifier matches · {reviewed} reviewed</p></div><button onClick={downloadReview}>Export review choices</button></div>
      <p className={s.small}>{batch.referenceLabel}</p>
      {batch.warnings.map(w => <p key={w} className={s.warning}>{w}</p>)}
      <div className={s.workspace}>
        <aside className={s.sidebar}>
          <label>Show<select disabled={applying} value={filter} onChange={e => setFilter(e.target.value)}><option value="all">All questions</option><option value="attention">Needs attention</option><option value="pending">Not reviewed</option></select></label>
          <nav aria-label="Imported questions">{visible.map((q,i) => <button key={q.id} aria-current={item?.id === q.id ? 'true' : undefined} disabled={applying} onClick={() => { setActive(q.id); setConfirmed(false); setNotice(''); }}><strong>{i+1}. {q.id}</strong><span>{q.matches.length === 1 ? q.matches[0].code : q.matches.length ? `${q.matches.length} possible matches` : 'No identifier match'}</span><span>{decisions[q.id]?.preference ?? 'Not reviewed'}</span></button>)}</nav>
        </aside>
        {item ? <div className={s.detail}>
          <div className={s.sectionHead}><div><h3>{item.id}</h3><p>{item.matches.length ? 'Identifier match — confirm the content is identical before preferring a replacement.' : 'No identifier match. Content-based duplicate checking is still required.'}</p></div><div className={s.toggles}><label><input type="checkbox" checked={reveal} onChange={e=>setReveal(e.target.checked)} /> Show answers & explanations</label>{pdfUrl && <button onClick={()=>setSourceOpen(!sourceOpen)}>{sourceOpen ? 'Hide source PDF' : 'Show source PDF'}</button>}</div></div>
          {item.matches.length > 1 && <label>Existing question to compare<select disabled={applying} value={matches[item.id] ?? ''} onChange={e=>{setConfirmed(false);setMatches({...matches,[item.id]:e.target.value});setDecisions(current=>{const next={...current};delete next[item.id];return next;});}}><option value="">Select a match</option>{item.matches.map(m=><option key={m.id} value={m.id}>{m.code} {m.deleted ? '(deleted)' : ''}</option>)}</select></label>}
          ...
          <div className={s.decisions}><strong>Rendering preference</strong><div>{['Keep existing','Prefer imported','Needs editing'].map(value=><button key={value} aria-pressed={decisions[item.id]?.preference === value} disabled={applying || !!(match && applied[match.id]) || (!match && value !== 'Needs editing')} onClick={()=>choose(value)}>{value}</button>)}</div><p className={s.small}>Choosing a preference does not change the bank. Preferences stay in this session until exported; use the separate apply step to replace a presentation.</p><p role="status">{notice}</p></div>
          {match && decisions[item.id]?.preference === 'Prefer imported' && <section className={s.upload} aria-label="Apply replacement">
            ...
            {applied[match.id] ? <p role="status">Applied to {match.code}. Compare again to review the current bank content.</p> : match.applyBlocked ? <p className={s.warning}>{match.applyBlocked}</p> : <>
              <label><input type="checkbox" disabled={applying} checked={confirmed} onChange={e => setConfirmed(e.target.checked)} /> I compared the text, choices, figures, and explanation and confirm their meaning is identical.</label>
              <p><button className={s.primary} disabled={!confirmed || applying} onClick={applyCurrent}>{applying ? 'Applying…' : `Apply imported presentation to ${match.code}`}</button></p>
