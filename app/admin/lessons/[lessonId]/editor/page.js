...
import {
  ...
  lessonTemplates,
} from '../../../../../lib/lesson/template-registry.mjs';
import {
  applyImportedBlocks,
...
const IMPORT_MODES = [
  { value: 'append', label: 'Append to lesson' },
  { value: 'insert_after_selected', label: 'Insert after selected block' },
  { value: 'replace_all', label: 'Replace all blocks' },
];
...
function InternalLessonEditor() {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState('graph_comparison_workflow');
  ...
  const templatePreviewBlocks = useMemo(() => {
    const template = lessonTemplates[selectedTemplate];
    if (!template) return [];
    const existingIds = blocks.map((block) => block.id).filter(Boolean);
    try {
      return template.create(templateParams, { existingIds });
    } catch {
      return [];
    }
  }, [selectedTemplate, templateParams, blocks]);
  ...
  function insertSelectedTemplate() {
    ...
    if (!template) return;
    const existingIds = blocks.map((block) => block.id).filter(Boolean);
    const generated = template.create(templateParams, { existingIds });
    const next = insertTemplateBlocks(blocks, selectedIndex, generated);
    setBlocks(next);
    setSelectedIndex(Math.min(selectedIndex + 1, next.length - 1));
    setDirty(true);
  }
  ...
  function validateImportSpec() {
    ...
    if (parsed.error) {
      setImportResult({
        lessonMetadata: {},
        blocks: [],
        issues: [{ severity: 'error', path: '$', message: `Invalid JSON: ${parsed.error}` }],
      });
      return;
    }
    const result = compileLessonTemplateSpec(parsed.spec, {
      existingBlockIds: blocks.map((block) => block.id).filter(Boolean),
      existingContentIds: blocks.map((block) => block.content?.id).filter(Boolean),
    });
  ...
  function applyImportResult() {
    ...
    if (importResult.issues.some((entry) => entry.severity === 'error')) return;

    const nextBlocks = applyImportedBlocks(blocks, importResult.blocks, importMode, selectedIndex);
    setBlocks(nextBlocks);
    if (importResult.lessonMetadata?.title || importResult.lessonMetadata?.description) {
      setLesson((prev) => ({
        ...prev,
        title: importResult.lessonMetadata.title || prev.title,
        description: importResult.lessonMetadata.description || prev.description,
      }));
    }
    setDirty(true);
  }
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div className="card" style={{ padding: 10, marginBottom: 12 }}>
        <h3 style={{ margin: '0 0 8px', fontSize: 14 }}>Templates</h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 280px) 1fr auto', gap: 8, alignItems: 'end' }}>
          <label style={{ fontSize: 12 }}>
            Template
            <select value={selectedTemplate} onChange={(e) => setSelectedTemplate(e.target.value)} style={{ width: '100%' }}>
              {Object.entries(lessonTemplates).map(([key, value]) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 8 }}>
            {(selectedTemplate === 'graph_comparison_workflow' || selectedTemplate === 'slider_workflow') && (
              <label style={{ fontSize: 12 }}>workflowId
                <input value={templateParams.workflowId || ''} onChange={(e) => setTemplateParams((prev) => ({ ...prev, workflowId: e.target.value }))} style={{ width: '100%' }} />
              </label>
            )}
            {selectedTemplate === 'graph_comparison_workflow' && (
              <>
                <label style={{ fontSize: 12 }}>originalExpression
                  <input value={templateParams.originalExpression || ''} onChange={(e) => setTemplateParams((prev) => ({ ...prev, originalExpression: e.target.value }))} style={{ width: '100%' }} />
                </label>
                <label style={{ fontSize: 12 }}>candidateExpression
                  <input value={templateParams.candidateExpression || ''} onChange={(e) => setTemplateParams((prev) => ({ ...prev, candidateExpression: e.target.value }))} style={{ width: '100%' }} />
                </label>
              </>
            )}
            {selectedTemplate === 'branching_question' && (
              <>
                <label style={{ fontSize: 12 }}>baseId
                  <input value={templateParams.baseId || ''} onChange={(e) => setTemplateParams((prev) => ({ ...prev, baseId: e.target.value }))} style={{ width: '100%' }} />
                </label>
                <label style={{ fontSize: 12 }}>prompt
                  <input value={templateParams.prompt || ''} onChange={(e) => setTemplateParams((prev) => ({ ...prev, prompt: e.target.value }))} style={{ width: '100%' }} />
                </label>
              </>
            )}
          </div>

          <button className="btn secondary" onClick={insertSelectedTemplate}>Insert Template</button>
        </div>
        <div style={{ marginTop: 8, fontSize: 11, color: 'var(--muted)' }}>
          Preview: {templatePreviewBlocks.length} block(s) will be inserted after selected block.
        </div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 6 }}>
          {templatePreviewBlocks.map((block, idx) => (
            <span key={`${block.id}-${idx}`} style={{ fontSize: 11, background: 'var(--bg-alt, #f3f4f6)', borderRadius: 12, padding: '2px 8px' }}>
              {idx + 1}. {block.block_type} ({block.id})
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 10, marginBottom: 12 }}>
        <h3 style={{ margin: '0 0 8px', fontSize: 14 }}>Import from Template Spec</h3>
        <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
          <button className="btn secondary" onClick={() => setImportSpecText(IMPORT_EXAMPLES.equivalent_expressions)}>Load example: Equivalent Expressions</button>
          <button className="btn secondary" onClick={() => setImportSpecText(IMPORT_EXAMPLES.boundaries)}>Load example: Boundaries</button>
        </div>
        <textarea
          value={importSpecText}
          onChange={(e) => setImportSpecText(e.target.value)}
          spellCheck={false}
          style={{ width: '100%', minHeight: 180, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: 12, borderRadius: 6, border: '1px solid var(--border, #ddd)', padding: 10 }}
        />
        <div style={{ display: 'flex', gap: 8, marginTop: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <label style={{ fontSize: 12 }}>
            ...
            <select value={importMode} onChange={(e) => setImportMode(e.target.value)} style={{ marginLeft: 8 }}>
              {IMPORT_MODES.map((mode) => <option key={mode.value} value={mode.value}>{mode.label}</option>)}
            </select>
          </label>
          <button
            className="btn primary"
            onClick={applyImportResult}
            disabled={!importResult || importResult.issues.some((entry) => entry.severity === 'error')}
          >
            Import
          </button>
        </div>
            ...
            <div style={{ marginTop: 6, display: 'grid', gap: 6 }}>
              {importResult.issues.map((entry, idx) => (
                <div key={`issue-${idx}`} style={{ border: '1px solid var(--border, #ddd)', borderRadius: 6, padding: 6 }}>
                  <div style={{ fontWeight: 700, color: entry.severity === 'error' ? 'var(--danger)' : '#b26a00' }}>
                    {entry.severity.toUpperCase()} · {entry.path}
                  </div>
                  <div>{entry.message}</div>
                  {entry.suggestion && <div style={{ color: 'var(--muted)' }}>fix: {entry.suggestion}</div>}
                </div>
              ))}
            </div>
            <div style={{ marginTop: 8, display: 'grid', gap: 4 }}>
              {importResult.blocks.map((block, idx) => (
                <div key={`import-preview-${idx}`} style={{ border: '1px solid var(--border, #ddd)', borderRadius: 6, padding: 6 }}>
                  <strong>{idx + 1}. {block.block_type}</strong> · id: <code>{block.id || '—'}</code> · content.id: <code>{block.content?.id || '—'}</code>
                  {block.content?.workflow_id && <> · workflow: <code>{block.content.workflow_id}</code></>}
                  {(block.content?.on_correct_block_id || block.content?.on_incorrect_block_id) && (
                    <> · branch: <code>{block.content.on_correct_block_id || '—'}</code>/<code>{block.content.on_incorrect_block_id || '—'}</code></>
                  )}
                </div>
              ))}
            </div>
