...
export function PracticeInteractive({
  ...
  // Local UI state for the in-flight selection. Once the student
  // submits, we flip `result` and lock the selection.
  ...
  const handleSubmit = useCallback(
    async (event) => {
      ...
      if (!selectedId) {
        setSubmitError('Please select an option first.');
        return;
      }
      setSubmitError(null);
      setSubmitting(true);
      try {
        const res = await submitAnswerAction(session.sessionId, session.position, selectedId);
        if (res && res.ok) {
          setResult({
    ...
    [submitAnswerAction, session.sessionId, session.position, selectedId, submitting, isReviewed],
  );
    ...
    <main style={S.main}>
      ...
      <article style={S.article}>
        ...
        <form onSubmit={handleSubmit} style={S.form}>
          ...
          {!isReviewed && (
            <button type="submit" disabled={submitting || !selectedId} style={S.submitBtn}>
