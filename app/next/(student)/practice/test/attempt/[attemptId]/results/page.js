// Practice-test results page — detailed Score Report.
//
// Everything the legacy score-report route produced, rebuilt on
// the v2 schema and the new-tree pattern:
//   - Composite + per-section scaled scores (hero card)
//   - Domain / skill accuracy breakdown (Math + RW columns)
//   - Opportunity Index (skills with the most room to grow,
//     combining learnability × wrong-question weights)
//   - Timing overview (total time, slowest 5 per section)
//   - Full per-question review grid with reveal-on-click
//   - PDF export (uses the existing lib/generateScoreReportPdf
//     which accepts the shared score-report shape)
//   - Floating Desmos calculator for math questions
//
// All heavy lifting runs server-side; the client island just
// renders + owns the selected-question + reveal state + PDF
// trigger.
...
export default async function PracticeTestResultsPage({ params }) {
  ...
  // 2) Module attempts, module items, item attempts (with
  //    attempts rows for answers + timing), student + teacher
  //    profiles, and skill-learnability in parallel.
  const [
    { data: moduleAttempts },
    { data: studentProfile },
  ...
  // Module ids so we can pull items + ordinals per module.
  ...
  // 3) Pull every question referenced, in one IN query. v2 has
  //    skill_code in addition to domain_code / domain_name /
  //    skill_name — needed for the OI join.
  ...
  // Index item attempts by item id for O(1) lookups below.
  const itemAttemptByItemId = new Map();
  for (const ia of itemAttempts ?? []) {
    itemAttemptByItemId.set(ia.practice_test_module_item_id, ia);
  }

  // Items per module, sorted by ordinal, tagged with a global
  // ordinal (1..N across all modules in test order).
  ...
  for (const [k, arr] of itemsByModule) {
    arr.sort((a, b) => a.ordinal - b.ordinal);
  }

  // 4) Build the flat per-question view model in test order
  //    (RW m1 → RW m2 → MATH m1 → MATH m2). Carry the module's
  //    subject + number so the OI + timing + PDF payload can
  //    group by them.
  const reviewItems = [];
  let globalOrdinal = 0;
  for (const ma of moduleAttemptList) {
    ...
    for (const it of items) {
      ...
      const stimulusHtml = applyWatermark(q.stimulus_rendered ?? q.stimulus_html, user.id);
      const stemHtml     = applyWatermark(q.stem_rendered ?? q.stem_html, user.id);
      ...
      const options = optionsSource.map((opt, idx) => {
        ...
        return { id: label, ordinal: idx, label, content_html: applyWatermark(content, user.id) };
      });
      ...
      reviewItems.push({
        ...
        reveal: {
          ...
          rationaleHtml: applyWatermark(q.rationale_rendered ?? q.rationale_html, user.id),
        },
        status: a ? (a.is_correct ? 'correct' : 'incorrect') : 'unanswered',
        marked: !!ia?.marked_for_review,
      });
  ...
  // 5) Domain / skill accuracy grouped by subject.
  const domains = buildDomainStats(reviewItems);

  // 6) Section scoring summary (from the attempt row + per-
  //    section item counts for the "X/Y correct" sublabel).
  const sections = buildSections(attempt, reviewItems, moduleAttemptList);

  // 7) Opportunity Index — one row per skill that has missed
  //    items, weighted by (learnability × ease × module-route).
  ...
  // 10) PDF data payload — shaped exactly like the legacy route's
  //     JSON so lib/generateScoreReportPdf can render it without
  //     modification.
...
function buildSections(attempt, items, moduleAttemptList) {
  ...
  // Route per subject is module 2's route_code.
...
function buildDomainStats(items) {
  // domain_name → { subject_code, domain_name, correct, total, skills: Map }
...
function buildOpportunity(items, sections, learnMap) {
  ...
  const oi = new Map(); // skill_code → accumulator
  for (const it of items) {
    if (it.missing) continue;
...
// ──────────────────────────────────────────────────────────────
// PDF payload — shaped to match the legacy JSON that
// generateScoreReportPdf already expects.
// ──────────────────────────────────────────────────────────────
...
// Silence unused-import warning for domainSection — imported in
// case follow-ups want to split the PDF by subject via the code.
// eslint-disable-next-line no-unused-vars
const _keep = domainSection;
