// Tutor → training tests hub. Mirrors /practice/tests for the
// teacher's own data: same stats strip + launcher + recent
// composites chart + history table.
//
// All practice_test_attempts_v2 rows are user-scoped, so a
// teacher's launches and completions just live alongside their
// student-side counterparts. The TestLauncher links into the
// per-test instruction page at /practice/test/[testId] (the
// existing route works for any role with can_view on the test);
// the runner + results pages are reused too. The training tree
// owns the hub framing and the launch entry, the rest of the
// per-test flow is shared infra.

import { redirect } from 'next/navigation';
import Link from 'next/link';
import { requireUser } from '@/lib/api/auth';
import { TestLauncher } from '@/lib/practice/TestLauncher';
import s from './TrainingTests.module.css';

export const dynamic = 'force-dynamic';

export default async function TutorTrainingTestsPage() {
  const { user, profile, supabase } = await requireUser();

  if (profile.role === 'student' || profile.role === 'practice') redirect('/practice/tests');
  ...
  return (
    <main className={s.container}>
      <header className={s.header}>
        <div className={s.eyebrow}>Train · Practice tests</div>
        <h1 className={s.h1}>Practice tests</h1>
        <p className={s.sub}>
          Full-length SAT simulations under timed conditions. Your own
          training data — separate from the rosters you teach.
        </p>
      </header>
        ...
        <div className={s.cardHeader}>
          <div>
            ...
            <div className={s.cardHint}>
              ...
              You&apos;ll see the module summary before the timer
              starts.
            </div>
          </div>
        </div>
...
function buildStats(completedAttempts) {
  ...
  const bestRw = withScore.reduce((m, a) => Math.max(m, a.rw_scaled ?? 0), 0);
  const bestMath = withScore.reduce((m, a) => Math.max(m, a.math_scaled ?? 0), 0);
  return {
    taken: completedAttempts.length,
  ...
  return (
    <div className={s.statsStrip}>
      <StatTile
        label="Tests taken"
        value={stats.taken}
        sub={stats.taken === 0 ? 'First one is waiting for you' : 'Full-length practice'}
      />
      <StatTile
        label="Latest composite"
        value={stats.latestComposite == null ? '—' : stats.latestComposite.toLocaleString()}
        sub={
          stats.delta == null ? 'Your most recent score'
          : stats.delta === 0 ? 'Steady — no change from last test'
          : stats.delta > 0 ? `↑ ${stats.delta} from previous`
      ...
      <StatTile
        ...
        value={stats.bestComposite == null ? '—' : stats.bestComposite.toLocaleString()}
        sub={
          stats.bestRw != null && stats.bestMath != null
      ...
      <StatTile
        ...
        sub={
          stats.delta == null ? 'Finish two tests to see a trend'
          : stats.delta > 0 ? 'You are trending up'
          : stats.delta < 0 ? 'Drill the gaps then come back'
...
function TrendChart({ attempts }) {
  ...
  return (
    <div className={s.chartWrap}>
      <svg viewBox={`0 0 ${W} ${H}`} className={s.chart} role="img" aria-label="Recent composite scores">
        {[lo, Math.round((lo + hi) / 2), hi].map((tick) => {
          const y = PAD_Y + innerH - ((tick - lo) / range) * innerH;
          return (
            <g key={tick}>
              <line x1={PAD_X} x2={W - PAD_X} y1={y} y2={y} stroke="currentColor" strokeOpacity="0.12" strokeDasharray="2 4" />
              <text x={PAD_X - 6} y={y + 4} textAnchor="end" fontSize="10" fill="currentColor" opacity="0.5">
                {tick}
              </text>
            </g>
          );
        ...
        {attempts.map((a, i) => {
          ...
          return (
            <g key={a.id}>
              <rect x={barX} y={y} width={barW} height={Math.max(h, 2)} fill="var(--color-app-accent, #0a66c2)" opacity="0.85" rx="3" />
              <text x={cx} y={y - 6} textAnchor="middle" fontSize="11" fontWeight="700" fill="currentColor">
                {a.composite_score}
              </text>
              <text x={cx} y={PAD_Y + innerH + 14} textAnchor="middle" fontSize="10" fill="currentColor" opacity="0.6">
                {label}
              </text>
            </g>
          );
            ...
            <tr key={a.id}>
              <td className={s.td}>
                ...
                <div className={s.testMeta}>
                  ...
                  {a.time_multiplier && a.time_multiplier > 1 ? ` · ${a.time_multiplier}× time` : ''}
                </div>
              </td>
              <td className={s.td}>{formatDate(a.finished_at)}</td>
              <td className={s.tdNum}>{Number.isFinite(a.rw_scaled) ? a.rw_scaled : '—'}</td>
              <td className={s.tdNum}>{Number.isFinite(a.math_scaled) ? a.math_scaled : '—'}</td>
              <td className={`${s.tdNum} ${s.tdComposite}`}>
                {Number.isFinite(a.composite_score) ? a.composite_score : '—'}
              </td>
              <td className={s.tdAction}>
                <Link href={`/practice/test/attempt/${a.id}/results`} className={s.viewLink}>
                  View report →
                </Link>
              </td>
            </tr>
